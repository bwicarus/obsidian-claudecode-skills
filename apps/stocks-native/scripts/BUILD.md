...
Signing uses the existing valid distribution identity, matches it to the Apple team's certificate, and registers the exact new bundle only if absent. It creates a distribution profile only when no active matching profile exists. It never creates/revokes certificates, deletes profiles, or changes Reader capabilities. First signing can require Apple API permissions that the existing key may not possess; the macOS run is the live check.
...
Current local verification: Python syntax and workflow structure can be checked on Windows. Actual Xcode compilation, certificate validity, profile permissions and IPA export must be verified by the macOS run. No workflow has been pushed or dispatched by the build subtask.
