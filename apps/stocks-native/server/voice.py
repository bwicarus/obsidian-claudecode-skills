...
制卡的完整save示例在当前工作区 .agents/skills/stocks-planning/SKILL.md；catalog只列枚举及顶层字段，不要猜档位结构。每个variant只允许id,label,action,targetPrice,targetKind,suggestedShares,urgency,reason,rules，其中id/label/action/reason必填；条件和理由写reason，不添加entry/stopLoss/summary等额外键。具体阈值写rules:[{type,value}]，无依据可空数组；无目标价填targetPrice:null,targetKind:无，无仓位依据suggestedShares:null，urgency使用normal/warn/critical。
