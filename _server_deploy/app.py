...
PROTECTED_PREFIXES = ("/dashboard", "/private", "/history", "/qa", "/profile", "/admin", "/auth", "/control", "/pdf", "/insights",
                      "/api/assistant", "/api/fitness", "/api/kg", "/kj", "/api/ambient")   # /kj=KJ 知识节点(2026-09-06,外部 agent 只有 token);后三个:让 Bearer 桥也覆盖(MCP 外部 agent 冷启动直调健身/助手,此前靠先调 /pdf 拿 session cookie 的隐式顺序;/api/kg 是电脑侧只读拉图谱,只有 token 没有 cookie)
...
# 环境旁听的 jev 判断(/api/ambient/*):iPad 送带说话人标签的转写,服务器持 jev 密钥判断(2026-09-26)
from ambient_jev import register_ambient
