...
def _sys_prompt(ctx):
    ...
    return (
        ...
        "优先针对**选中**直接回答/查词/翻译/解释/语法/制卡(read_selection 拿全文,或直接用标注里的选中文字),"
        "**别去 see_page 看图**;除非用户**明确**说『这张图/这幅图/这页的图/图里画的』才看图。\n"
        "★只有用户**明确**要看图(问『这张图/这个图表/这页的图/看一下这页/这里画的是什么』,或公式排版/手写等文字层读不到)"
        "时才用 **see_page**;**别因为『页面里有图』就主动去看图**(漫画/插图书每页都有图,但用户多半在问文字/选中)。\n\n"
        f"【可用工具】\n{cat}\n\n"
