...
window.__upReconcileDelete = function (newMeta) {
  try {
    if (!_imgMode || readMode !== 'continuous') return false;   // 非图片/非连续:退回 reload(spread 行分组/canvas 字节都动不了)
    ...
    if (kids.length !== M) return false;   // DOM 页数跟后端不一致(有未完成插入 job / spread 残留等)→ 稳妥退回 reload
    ...
    return true;
  } catch (e) { try { window.dlog && window.dlog('reconcile fail: ' + e.message, '#ff6b6b'); } catch (_) {} return false; }
};
