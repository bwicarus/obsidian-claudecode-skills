...
(() => {
  ...
  function curLoadedPage() {                                 // 视口中线页,且内容真的渲染出来了
    let page = 0, best = -1;
    const mid = window.innerHeight / 2;
    document.querySelectorAll('.page-wrap[data-page-num], .pdf-upage[data-page-num]').forEach((pw) => {
      ...
      if (ov > best) {
        ...
        page = ok ? (parseInt(pw.dataset.pageNum, 10) || 0) : 0;   // 没渲染出来 → 0 = 这秒不计
      }
    });
    return page;
  }
  ...
  function flush(useBeacon) {
    ...
    entries.forEach(([p]) => delete acc[p]);
    const body = JSON.stringify({ file: FILE_REL, dwell: entries.map(([p, s]) => ({ page: +p, secs: s })) });
    try {
