...
(function () {
  ...
  function mountModel(host, before, tool, md) {
    ...
    var selB = box.querySelector('.vc-md-b'), selV = box.querySelector('.vc-md-v'),
        selD = box.querySelector('.vc-md-d'), fastBtn = box.querySelector('.vc-md-fast'), st = box.querySelector('.vc-md-st');
    var dflt = md.default || {};
    function cur() {   // 有预设用预设,没有就是出厂默认
      var p = md.pref || {};
      return { backend: p.backend || dflt.backend, variant: p.variant || dflt.variant,
        depth: p.depth || dflt.depth, fast: p.fast === true || (!(p && p.backend) && dflt.fast === true) };
    }
    function fill(sel, list, val, disabled, labels) {
      sel.innerHTML = '';
      (list || []).forEach(function (x) {
        var o = document.createElement('option'); o.value = x; o.textContent = (labels && labels[x]) || x;
        if (labels && labels[x]) o.title = labels[x];
        if (disabled && disabled.indexOf(x) >= 0) o.disabled = true;
        if (x === val) o.selected = true;
        sel.appendChild(o);
      });
    }
    function codexUnselectable() {
      var caps = md.codex_capabilities || {};
      return ((md.variants || {}).codex || []).filter(function (model) {
        return !caps[model] || caps[model].selectable !== true;
      });
    }
    function codexFastSupported(model) {
      var cap = (md.codex_capabilities || {})[model] || {};
      return cap.selectable === true &&
        (cap.fast === true || cap.priority === true);
    }
    function depths(backend, variant) {
      if (backend === 'codex') {
        var byModel = md.codex_depths_by_model || {};
        if (Array.isArray(byModel[variant])) return byModel[variant].slice();
      }
      return ((md.depths || {})[backend] || []).slice();
    }
    function paint() {
      var c = cur();
      ...
      var labels = {};
      var caps = md.codex_capabilities || {};
      ((md.variants || {}).codex || []).forEach(function (model) {
        var cap = caps[model] || {};
        var reason = String(cap.reason || '');
        labels[model] = model + (reason
          ? ' · ' + reason
          : ((cap.fast === true || cap.priority === true)
            ? ' · 支持 Fast'
            : ''));
      ...
      fill(selV, (md.variants || {})[c.backend] || [], c.variant,
        c.backend === 'codex' ? codexUnselectable() : [], labels);
      fill(selD, depths(c.backend, c.variant), c.depth);
      var supportsFast = c.backend === 'codex' &&
        codexFastSupported(c.variant);
      fastBtn.disabled = !supportsFast;
      fastBtn.classList.toggle('on', supportsFast && c.fast === true);
      fastBtn.textContent = supportsFast && c.fast === true ? '⚡ Fast 开' : '⚡ Fast';
      fastBtn.title = supportsFast ? '只加速这个工具的 Codex 调用' :
        (c.backend === 'codex' ? '当前 Codex 型号不支持 Fast' : '仅 Codex CLI 可使用 Fast');
    ...
    paint();
    function save() {
      var fast = !fastBtn.disabled && fastBtn.classList.contains('on');
      fetch('/api/assistant/tool-prompt', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tool: tool, op: 'model', backend: selB.value, variant: selV.value, depth: selD.value, fast: fast }) })
        .then(function (r) { return r.json(); })
        .then(function (r) {
          if (!r || !r.ok) { toast((r && r.error) || '保存失败'); return; }
          md.pref = r.pref || {};
    ...
    selB.addEventListener('change', function () {   // 换后端 → 型号/深度目录跟着换,先落到该后端第一项
      var vs = (md.variants || {})[selB.value] || [];
      var first = vs[0] || '';
      if (selB.value === 'codex') {
        first = vs.find(function (model) {
          var cap = (md.codex_capabilities || {})[model] || {};
          return cap.selectable === true;
        }) || first;
      }
      var ds = depths(selB.value, first);
      md.pref = { backend: selB.value, variant: first, depth: ds[0] || '', fast: false };
      paint(); save();
    });
    selV.addEventListener('change', function () {
      if (selB.value !== 'codex' || !codexFastSupported(selV.value)) fastBtn.classList.remove('on');
      var ds = depths(selB.value, selV.value);
      md.pref = {
        backend: selB.value,
        variant: selV.value,
        depth: ds.indexOf(selD.value) >= 0 ? selD.value : (ds[0] || ''),
        fast: fastBtn.classList.contains('on')
      };
      paint(); save();
    });
    selD.addEventListener('change', save);
    fastBtn.addEventListener('click', function () {
      if (fastBtn.disabled) return;
      fastBtn.classList.toggle('on');
      fastBtn.textContent = fastBtn.classList.contains('on') ? '⚡ Fast 开' : '⚡ Fast';
      save();
    });
  }
  ...
  function mkInflow(chip) {
    ...
    if (!vc || !th || !vc.renderInflow) return null;
    // 统一壳(用户拍板 2026-07-21):侧栏工具卡也走 _renderInflow=同一 _cardDom → 与浮层/钉入/结果卡同长相+三态圆点
    //   (原手抄 innerHTML + 两态折叠退役)。renderInflow 已管:壳/vc-typed/--vc-tc/去▶✕/内联/头部三态循环+append。
    //   侧栏工具卡 = 对话记录 → 起手长条(min,步骤在长条滚);不要 ▶/✕(renderInflow 默认已去)。
