...
def _role_text(cfg: dict, book: dict | None, file_rel: str, page: int) -> str:
    ...
    role += ("\n你直接接着这本阅读器的工具层(目录在最后)。规则(与系统里另一个编排助手完全同一套):"
             ...
             "\n- **你没有通用网页搜索能力**:search_all_books 搜的是**用户自己的书库**,不是互联网;"
             "但 search_image(搜真实图片)/search_video(搜教学视频)是真联网工具,用户想看图片/视频就用它们。"
...
def _oa_instructions(book: dict, file_rel: str, page: int) -> str:
    ...
    parts = [cfg.get("rt_instructions") or cfg.get("system_role") or
             ...
             "工具描述里写了 args 字段的用法,照着填。你没有**通用网页搜索**能力(search_all_books 只搜他的书库),"
             "要网上实时信息就如实说没法查;但 search_image(搜真实图片)/search_video(搜教学视频)是**真联网工具**,"
             "用户想看图片/视频就调它们,结果自动显示在界面上;**绝不要自己输出 markdown 图片/链接占位符假装贴图**。"
