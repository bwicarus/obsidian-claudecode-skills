...
iPad 麦克风（平时）/ 通话引擎上行旁路（通话中）
   ...
   ├─ 16 kHz → FluidAudio Sortformer（fastV2_1，4 槽位：「我」+ ≤2 熟人 + 陌生人）
...
| `ios/BWReader/App/NativeAmbientSupport.swift` | 日志出口、服务器请求、Sortformer 模型、流式分离、声纹与熟人、重采样 |
...
- **熟人**：设置里「最近听到的人」→ 起名 → 从环形缓冲取他 ≥3 秒有效语音存 `voiceprint/people/`，
  窗口清空后换分离器预登记（`overwritingAssignedSpeakerName: false`，声音太像已有的人就不占位并出声）。
  Sortformer 只有 4 个槽位，所以熟人最多预占 2 个；**更多熟人要换成声纹嵌入比对**（FluidAudio 的
  WeSpeaker/CAM++ 嵌入 + 余弦相似度，未做）。
- **原声不出设备**：只有转写文字发服务器；不支持本机识别的语言直接不启动（不退回苹果服务器识别）。
- **模型不打包**：Sortformer 首次使用时从 HuggingFace 下载，之后本机缓存。
...
手表 / 眼镜接入；熟人超过 2 位的嵌入比对；把旁听摘要主动注入 CLI 语音的 jev 上下文（现在只经 MCP `voice_brief`）；
