...
## 分段转写：一人一段、按人用语言（2026-09-26 第三批）

用户：「在语音的非活跃时间切分，最好识别到每个人的开始和终止，对这段声音分辨：登记了语言就用该语言，不能就需要确认」。

- 不再用一条连续识别流（一种语言转全部人，再按时间把字分给人）。改为：分离器**已定稿**的片段 →
  同一人相邻（间隔 ≤0.8 秒）合成一段 → 定稿线越过段尾 0.6 秒才算说完 → 从环形缓冲切出（前后各 0.15 秒）
  → `NativeSegmentTranscriber`（串行、强制本机识别）。一段最长 28 秒；分离器没就绪时退回按音量切、不分人。
- 用哪种语言：「我」/ 不分人 → 设置里的「我的语言」；认出的人 → 人物页登记的语言（服务器 `speakers.json`
  `persons[id].language`，经 `/voiceprints` 下发）；都没有 → 候选语言各转一次，按
  「识别置信度 × NaturalLanguage 判断这段文字属于该语言的概率」挑最高（单看置信度不行：用错语言的识别器
  转出的乱码置信度也可能不低）。同一声音块推测一致 ≥3 次且占 ≥75% 后直接沿用。
- 推测结果带 `lang` + `langConfirmed=false` 送服务器，累计在声音块的 `langVotes`；人物页显示票数，一键「确认他说日语」写入登记。
- 代价：每段一次本机识别；未登记语言的人在前几段要按候选数各跑一次（候选越多越耗电）。
- **重叠说话仍混在一起**（两人同时开口的那几秒各自的段里都有对方的声音）：真正的声源分离需要另接模型（FluidAudio 没有），未做。
