...
## 8. 修订（2026-09-19 用户拍板）：流出来的就是最终结果，收尾不再重画

> 「流式传输最终的结果就直接当作最终结果就好了，不需要重新整理」

**轮次容器的写入（`upsert_only`）一次历史事件都不发** —— 中途不发、**收尾也不发**。
侧栏里那一轮的最终形态，就是流式过程中已经呈现的那一份：工具卡头 + 本轮累计的几句
AI 正文 + App 画的生成物卡。新开 / 重开侧栏时按存储从头渲染，那条路一直是对的。

两条各自独立的证据把这个决定逼出来：

- **收尾重画确实会错**。用户实测「说完一瞬间消失，关闭侧边栏打开后正常显示」——
  存储是对的（库里三句话俱全）、从头渲染也是对的，错的只有收尾那一次权威重载：
  它把活的内容冲掉，却没把记录重新画上去。
- **中途重画会打断功能**。为了让合并当场可见而在每次写入都发事件的那一版，让侧栏
  在**工具正在执行的当口**重载，绑定的快照版本随之变化，卡片投递被判
  `BW_READER_REALTIME_OUTPUT_STALE` 直接拒收 —— 落库之后 51 毫秒工具就失败了。

所以 §2「服务端是唯一真相」的适用范围收窄为：**存储是真相，但"真相"不必每轮都回灌到
已经正确的实时视图上**。回灌只发生在从头渲染的时刻（打开/重开/切换）。

⚠ 代价（明说，别以后当成 bug 再查一遍）：另一台设备上开着的侧栏，看不到这一轮的
增量，要等它自己重开。这是用户知情选择的取舍 —— 一次正确的实时呈现，胜过一次
"为了同步而重画"带来的两类事故。
