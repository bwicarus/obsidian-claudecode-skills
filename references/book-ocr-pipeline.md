...
- **🗜 压缩按钮（2026-06）**：第四个按钮 = `scripts/compress_pdf.py`（`/api/compress-async` 起 detached 子进程，复用 book-preprocess 状态文件 → 进度条/刷新恢复/KillMode 全共用，phase=`compressing` 已进各 active 白名单）。**PyMuPDF 逐图重压**：`page.get_images` → `extract_image` → PIL 降采样(长边≤2400px,BICUBIC)+ JPEG 重压(q72) → `page.replace_image(xref, stream)` → `doc.save(garbage=4)` → qpdf 重线性化 → 原地替换。实测扫描书 **省 ~3/4**（part2 切片 11MB→2.5MB=22%），文字层完整。
