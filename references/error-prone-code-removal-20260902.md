...
## 2026-09-06 补：ReaderPC 接管失败会让整栈下线，安装器现在兜底再拉一次

- 现象：`--install --launch` 后新实例报「旧 ReaderPC 未完成正常退出；拒绝强制接管」自退，
  而旧实例其实已经按带外退出请求正常退出（只是慢于新实例的等待）→ 一个 ReaderPC 都不在，
  Flask / 桥 / 三个 sidecar 随之全停，直到有人手动拉。同一天撞了两次（05:19、05:27），
  另一次的报文是「旧 Direct 已退出，但没有可验证的清理成功回执；拒绝接管」。
- 处理：`package_readerpc_server.verify_running_generation` 在"一个进程都没有"这一支先用稳定
  启动器（HKCU Run 用的同一个 `start-readerpc.vbs`）再拉一次新 release，拉起来了也打 WARN 说明
