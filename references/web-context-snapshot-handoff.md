...
### 1. 30 秒断连看门狗（Windows 侧，已实现，待随 Direct 包发布）

正常 STOP 或连接结束仍立即停止媒体并归还开始前的 per-app 输入、输出路由；若这次清理因
瞬时媒体收口失败而仍持有路由租约，则只安排一次约 30 秒后的重试，不运行轮询器。
重试使用原连接 ID 作为 owner 代际围栏：期间若已有新连接取得音频所有权，旧看门狗只返回
no-op，不能停止或恢复新连接的路由；只有原连接仍是 owner 且无新活跃连接时才再次清理。
