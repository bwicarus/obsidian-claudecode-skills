...
## 额度消耗速率（2026-09-16 实测 + 官方 rate card 交叉验证）

数据来自 `%LOCALAPPDATA%\BWReaderoice-cli\quota-watch.jsonl`（每 ~2 分钟一条，
带 `weeklyPercent` / 会话状态 / `audioMs` / 累计 token），1588 个区间，跨 28.4 小时。
周窗口 10080 分钟，`usedPercent` 是**已用**百分比，套餐 `prolite`，无额外 credits。

⚠ **第一版算错过**：分子用了账号总额度，分母只用了后台文字模型的 token，
于是语音音频的开销被摊进了 token 单价里，两者没分开（用户当场指出）。
下面是按「涨点 ≈ a×语音分钟 + b×后台 Mtoken」非负最小二乘重算的结果。

| 来源 | 速率 | 独立验证 |
|---|---|---|
| **语音音频** | 1 点 ≈ **39 分钟** | 纯语音区间（有音频、后台没跑）128.8 分钟花 3 点 → 43 分钟/点 |
| **后台文字** | 1 点 ≈ **3.64 M token** | 账号 `today.tokens` 一天 39.4M ≈ 11 点，跟实际日耗吻合 |

这段记录里两者各占：**语音 40%、后台 60%**。

### 跟官方 rate card 对得上

[官方定价页](https://learn.chatgpt.com/docs/pricing)：「Voice in Desktop uses your
existing Codex usage budget at **$0.05 per minute**」，按 credit 计费的工作区是
**1.25 credits/分钟**。

拿实测的「1 点 ≈ 39 分钟」反推：1 点 ≈ 39 × 1.25 ≈ **49 credits**，
即这个账号的周额度 ≈ **4,900 credits**。两条独立路径给出同一个量级：
**只用语音、什么别的都不干，约 65 小时能把一周额度用光。**

网上找不到更细的：[openai/codex#42887 "Voice drains quota too quickly"](https://github.com/openai/codex/issues/42887)
只有定性抱怨（「even when no agent is actively working」），没有任何数字，也没有维护者回应。

### 实践含义

- 通话时长不是主要变量，**后台每轮读多少上下文才是**（同样时长耗额度能差 6 倍：
  9/16 凌晨 43.7 分钟只花 1 点，9/15 21 点 9.8 分钟花了 3 点）。
- 已经砍掉的每轮约 39K 字（Codex 样板 25.6K + 冷工具折叠 13.8K），
  按 3.64M token/点 折算，大约**每 93 轮省 1 个点**。
- 没在通话时约 0.3 点/小时（后台定时任务与通知），基本可忽略。

⚠ **探针本身要花钱，而且看不见**：2026-09-16 16–17 点那 3 个点是我探 app-server
...
### Terra 宽口径实测（13 任务，2026-09-16）

上一轮只有 3 题、且用的是随手写的提示。这轮改用**生产环境真实的
`backendThreadInstructions`**（1157 字），任务覆盖真实调用统计里的主要类别，
阅读器在线（料理师part2 第 26 页，正文未加载）。会改动书的任务只让它描述、不执行。

**terra/medium 逐题**

| 任务 | 表现 |
|---|---|
| 读状态 / 选中状态 / 多步组合 | ✅ 走快照，答案准确（书名、页码、无选中、页上卡片） |
| 页上卡片 | ✅ 走快照拿到 ダイオキシン —— 正是指令要求的做法（状态里已有就别另调工具） |
| 折叠工具参数表 ×2 | ✅ 参数全对（一个走 capability_guide，一个走代码模式） |
| 笔记 | ✅ 真列出 34 条 |
| 高亮 / 建卡（空跑） | ✅ 工具名与参数形状都对 |
| 书内搜索 / 目录 | 判定拿不到并如实说 —— **Astra 同样拿不到**，是环境限制不是模型差距 |
| 查词 | 没调词典、自己答了 —— 但 **Astra 也一样**，不是 Terra 的缺陷 |
| 取某张卡 | 理解成「念出来」去调 voice_say（题目本身有歧义），卡在没音频设备 |

**Astra 补测**（只补上面不干净的 4 题）：没有一题明显更强。
唯一差别是「取某张卡」它先查了快照、报出卡名再朗读，Terra 直接朗读。

**⚠ effort 要 medium，不能 low**：terra/low 在「读状态」这种最基本的题上都失手了
（去调了 `js` 而不是快照，最后答「无法获取」），整体更爱瞎试。
...
### 委托有没有「前置信号」：没有，而且内容预判也不划算（2026-09-16）

用户问能不能在委托之前就知道这句要不要后台干活 —— 因为「说完就注入」会把纯聊天也算进去。

**协议层面没有。** GPT-Live 官方文档的委托事件序列里，`session.delegation.created`
就是第一个 —— 它发出时模型已经决定了。之前的只有
`session.input_transcript.delta` / `session.output_transcript.delta`（转写流），
不带任何「打算委托」的意思。
⚠ 顺带：运行器的 `on_dc_message` **没有兜底分支**，白名单外的数据通道事件被静默丢弃，
所以「通道上只有 4 种事件」这个印象未必是真的，要找新信号得先加兜底记录。

**从说话内容预判也不划算。** 拿 477 句真实转写做过评测（真值 = 之后 40 秒内有没有委托）：

| 规则 | 精确率 | 召回率 | 漏判 |
|---|---|---|---|
| 指代词（这个/这句/选中…）或动作词（高亮/卡片/翻译…） | 54% | 36% | 167 句 |
| 现状：每句都投 | 55% | 100% | 0 句 |

关键是**基准率就有 55%**（477 句里 262 句真的走了后台），所以任何保住召回的规则
都不可能比「全投」精确多少。漏判样例也很说明问题：「现在是第几页」「哪本书的第二页」
这种明显要后台的句子都被规则漏掉。

⚠ 另更正一个数：早先说「963 次开口只有 316 次委托、三分之二是白投」是**算错的** ——
那把 `dc_turn_created` 和 `transcript` 重复计数了。按转写算是 477 句对 262 次委托，
纯聊天只占 45%。

**所以方向应该是压体量而不是压频次**：实测单次注入从 103 字到 13381 字不等，
小的投了不心疼，贵的是带整页正文那部分。

### 启用的 realtime/start 新参数（2026-09-16）

`realtimeEndInstructions`（收尾指令，配合 idleStopMinutes 自动关闭时好好道别）、
`flushTranscriptTailOnSessionEnd: true`（结束时刷出没落库的转写，自动关闭不丢最后一段）。
`realtimeStartInstructions` 与 `codexResponseItemPrefix` 留了设置位，默认不传。

发布前真开了一次会话验证不会被拒（`thread/realtime/start` 返回成功、stop 干净）。
⚠ 这只验证了**参数不被拒**，实际行为要等真实通话才看得到。
