...
## 额度消耗速率（2026-09-16 实测，1586 个采样区间）

数据来自 `%LOCALAPPDATA%\BWReader\voice-cli\quota-watch.jsonl`（每 ~2 分钟一条，
带 `weeklyPercent` / 会话状态 / 通话秒数 / 累计 token）。周窗口 10080 分钟，
`usedPercent` 是**已用**百分比，套餐 `prolite`，无额外 credits。

**结论：额度跟 token 走，不跟通话时长走。**

| 口径 | 速率 | 各小时之间的离散度 |
|---|---|---|
| 按 token | **1 点 ≈ 1.42 M token** | 48% |
| 按通话时长 | 1 点 ≈ 10 分钟通话（均值 6.1 点/小时） | **77%** |

同样的通话时长，耗额度能差 6 倍 —— 9/16 凌晨 43.7 分钟只花 1 点（1.4 点/小时），
9/15 21 点 9.8 分钟花了 3 点（18.5 点/小时）。差别全在后台文字模型那一轮读了多少上下文。
所以「语音每小时几个点」这种说法会把人带偏，**要控额度就去压每轮的上下文**，
不是去压通话时长。

参照：已经砍掉的每轮约 39K 字（Codex 样板 25.6K + 冷工具折叠 13.8K）
按 1.42M token/点 折算，大约每 36 轮省 1 个点。

没在通话时约 0.3 点/小时（后台定时任务与通知），基本可忽略。

⚠ **探针本身是要花钱的**：2026-09-16 16–17 点那 3 个点全是我探 app-server 时
起的真实模型轮烧掉的，而且那些轮跑在独立的 app-server 进程里，
**不计入运行器的 totalTokens**，只体现在账号百分比上。以后做这类探测要预先说明代价。
