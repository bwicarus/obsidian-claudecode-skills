...
### 🔴 Apple 官方定论：watchOS 后台双工音频**没有公开 API**

2026-08-27 查证，**两条一手来源，都已亲自核实原文**：

**① 论坛 thread 786754（2025-06），Apple DTS 工程师 Kevin Elliott 回复。**
提问者的配置**跟我们一字不差** —— `playAndRecord` + `voiceChat` +
`AVAudioEngine` + `AVAudioPlayerNode` + 网络收 RTP + Audio/VoIP 后台模式，
问怎么才能像原生对讲机那样在后台维持双工音频。回答两句：

> 问题 1：是的，watchOS 上 **CallKit 就是这么工作的**。
> 问题 2：**watchOS 目前没有公开 API 能让你的 app 做到这件事。**

**② 论坛 thread 750432，Apple 框架工程师。** 解释了「为什么没有报错」：

> 「在 watchOS 上，app 处于后台时**录音无法恢复**。必须由**前台的用户操作**
> 发起。（发起之后可以在后台继续。）」

### 由此确定的事故链

```
前期正常 → 某个系统事件中断音频（Siri / 来电 / 健身 / 系统音）
        → 后台无法重新激活 → 会话静默死亡
        → 豁免随之消失 → 数字停住 → 一个错都不报
```

**「边放边录后台能活」是真的，但它脆**：进后台没问题，**第一次中断就是终点**，
且不可恢复。中断是概率事件 —— 跑得越久越必然撞上。

### ✅ 顺带排除：不需要 AirPods

我怀疑过「watchOS 后台音频要求蓝牙路由」。查实了 —— 那条**只适用于
`.playback` 类别 + `longFormAudio` 路由策略**：

> `AVAudioSession.setCategory(_:mode:policy:options:)` 文档原话：
> 长音频路由策略只能配 `playback` 类别与 default/moviePlayback/spokenAudio 模式。

而 `.playAndRecord` + `.voiceChat` **根本没资格用那个策略**，我们必然走
`.default`，内置扬声器没问题。**这条产品层面的顾虑可以划掉。**

（顺带证实了另一件事：同一页写着「`activate(options:)` 才是**所有**类别的
授权机制，`setActive` 不是」—— 跟我们真机撞出来的那个静默陷阱对上了。）

### 于是取舍变成这样（用户 2026-08-27 已拍板）

| 路线 | 后台音频 | 我们自己的界面 |
|---|---|---|
| CallKit | ✅ Apple 唯一支持的路 | ❌ 通话中只显示系统通话 UI |
| 非 CallKit（现在这条） | ⚠ 撑到第一次中断 | ✅ |

**用户选择：接受降级，把死亡说清楚。** 即保留自己的界面，
用看门狗当场发现死亡并明说「抬腕回到 App 恢复通话」，
而不是让它悄悄冻住。
