...
- 自由卡与锚定卡在 AI 看来是同一种卡：当前页快照都直接包含稳定 placement ID、notes revision、
  卡片类型和完整的语义正文；锚定卡另有当前编号与锚定词 label。快照不得携带渲染 HTML、按钮、
  CSS/data 属性或图片代理地址。删除可直接使用快照里的 ID + revision；整体替换可由语义正文直接
  构造，只有局部编辑必须无损保留既有图片、链接或布局时，才通过带 revision 的连续分块读取精确源，
  并拒绝跨版本拼接。语义正文只保留防程序故障的异常大上限，不做日常摘要截断。
