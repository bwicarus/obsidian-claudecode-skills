...
**补记（2026-09-06，桥 0.1.293）**：钉住版只给"模型来读快照"那一个入口。凡是**动手**的工具
（`reader_visual_image` 取图、`reader_browser_control` 控制网页）读实时版 —— 取图看的是此刻的笔迹，
控制网页要等快照**前进**来确认到位，钉住版永远不前进。上线当天的实锤：说完话才画的圈，取图回
「没有可用合成图」，正是取图读了钉住版。高亮 / 建卡 / 查页 / 练习纸照旧读钉住版：模型是对着那版
挑的目标，写回也该落在那一页。契约：`snapshot-pin.contract.test.mjs` 按函数切片钉住这两处。
