...
- [x] 对话：轮次排序、操作标识、上下文选中及事件处理已接 Swift；原件、卡片/媒体移除通知和重复事件规则通过 Apple 原生检查。兼容生产者仍通过既有协议报告事件，不再从隐藏节点计算显示成员。
- [x] 复习：模式/取卡、改进草稿、删除及暂存评分效果已接原生服务；Apple 原生检查通过。本地评分和投递记录同事务保存，重试复用原回执，保留可撤销暂存、显式确认、未知结果不重发。
- [x] 账户投递：`ReaderNativeCommandOutbox` 已接 Swift 本地/远端发送分流和逐项回执；Apple 原生检查通过。兼容生产者仍交原命令入库，本地操作不出网，其他书籍延后。
- [x] 开书与阅读状态：旧数据启动导入、PDF 文件写者协调及恢复、ReaderPC 上下文组装/发布已接 Swift；Apple 原生检查通过。EPUB 正文及局部视频继续保留 WebKit。
- [x] 统一验证：原生合同检查及完整 App 模拟器构建通过（Actions `36051387988`）；词组、卡片显示、拖放、附件与输入法相关 Node 检查通过。实际 iPad 操作和性能尚未验证。
- [x] 出包：`eb3747d4` 已签名归档、导出并上传 TestFlight **1.1.85（974）**，Actions `36051387988` 全部成功，上传回执 `UPLOAD SUCCEEDED with no errors`。实机滚动/Pencil/多设备及弱网效果尚未验证，不凭编译宣称通过。

安装包已生成并上传。2026-09-25 05:33 JST 的 App Store Connect 查询（Actions
`36055643813`）确认 1.1.85（974）为 `VALID` / `IN_BETA_TESTING`，上传状态
`COMPLETE`，已分配内部测试组 `me`，可以通过该组的 TestFlight 账号安装。
外部状态为 `READY_FOR_BETA_SUBMISSION`，未提交外部测试审核。
弱网语音协议更换仍是调研项，不混入此次出包。

2026-09-25 统一构建记录：`d25321d3` 的完整 App 模拟器编译通过（Actions
`36050242858`），但四项原生检查失败，签名与上传未执行。`eb3747d4` 修正了测试样本的
卡片类型、视频编号与 Int64 断言，未放宽实际输入/版本校验。Actions `36051387988`
（构建 974）的全部原生检查与整包编译、签名、归档校验、IPA 导出及 TestFlight 上传均通过。
构建与上传证据：[Actions 36051387988](https://github.com/bwicarus/obsidian-claudecode-skills/actions/runs/36051387988)。
IPA 与构建日志：[构建产物](https://github.com/bwicarus/obsidian-claudecode-skills/actions/runs/36051387988/artifacts/10831562157)，Actions 默认保留 7 天。

发布边界额外运行的 `handoff_check.py` 并非全绿：Windows 发布源码白名单不接受现有
`computer-voice-desktop/tests/fixtures`，两处网页 fetch 缺少审计标记，扩展/PWA manifest
旧门禁与当前清单顺序不一致；日志 `C:\tmp\native-release-handoff.log`。没有据此发布
Windows 或独立扩展渠道。本次 iOS/Safari 包使用 Actions 自己的资源、归档与签名校验，
均已通过；不能把本次结果称为所有发布管线均通过。
