...
安装包已生成并上传。2026-09-25 05:17 JST 的 App Store Connect 查询（Actions
`36053870164`）确认 1.1.85（974）的上传状态为 `PROCESSING`，尚未出现可安装 build；
内部测试组 `me` 已配置接收全部构建。弱网语音协议更换仍是调研项，不混入此次出包。
