...
仍有迁移依赖：轮次创建、部分独立旧消息、placement、电脑通话设置/复习队列和阅读区。
...
成功。预设仍保存/应用/删除原服务的数据，本设备选项继续使用原键。独立电脑/Codex 通话
配置还待迁移。新增浏览器验证覆盖目录、禁用模型、Fast、失败回执、单字段语音保存与
本设备选项，三项浏览器用例及 2104 项 Node 合同通过；尚待本轮 Apple 编译。
