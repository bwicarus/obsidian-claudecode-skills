...
- **整页翻译 `_drawPageTranslate` = 重排阅读流（2026-06，最终方案）**：先后试过「逐句就地白底覆盖」「整页白底遮罩 + 字高量化 + 统一缩放」，**就地覆盖对正常段落 OK，但碰到表格/分栏/散落文本框（原文是日文式散排）会被原版面切得东一句西一句、阅读顺序全乱**（用户实测「阅读效果很差」截图为证）。结论：「整页翻译来读」的正解是**放弃精确版面、重排成干净中文阅读流**：
  1. **阅读顺序排序**：每句取首行 top / 平均垂直中心 `cyv` / 最左 `left`；按 `cyv` 聚类成视觉行（阈值=全页 rect 高**中位数** `medH`×0.6），同行内按 `left` 从左到右 → 表格按行读、不串行、不跨栏跳。
  2. **白底重排块**：一个 `.page-tr-reflow` div 盖在**全页文字 bbox**（所有句 rect 的 union）上，`background:#fff` 彻底遮原文，`min-height=原文区高`（中文比原文紧凑时底部原文也不露），`height:auto` 内容长则向下长。所有句子按阅读顺序作 `.page-tr-para` 块（各占一行、`margin-bottom:.4em`）顺下来，`line-height:1.6` 正常排版换行。
  3. **字号**：起始 `max(12, round(medH×sy×0.78))`（≈正文字高，medH 用**全页 rect 高中位数**，比「每句最小高」稳——后者被细碎 rect 拉到 14pt 偏小）。重排后 `scrollHeight > 原文区高×1.03` 则按 `sqrt(可用高/实际高)` 缩一档（min 11px），尽量塞回原区域不外溢太多。
  - 代价：表格/图文混排页失去精确单元格位置（表格变成按行的句子列表）、bbox 内的插图会被白底盖住——但换来可读性，符合用户「来读整页中文」的诉求。实测 p24（含攻击链表格 + 散落 worked-example）重排后 38 句从上到下顺、表格按行读，远好于就地覆盖的乱序。
  - 单句翻译 `_drawSentenceOverlay`（选一句译）**保持就地覆盖**（一次只一句、贴原文行位置有对照价值，无乱序问题）：`min(自然字高, floor(Wtot/(N+行数)))` 自适应字号 + 放不下换行兜底。
