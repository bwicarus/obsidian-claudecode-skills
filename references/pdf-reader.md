...
**逐字浮现接整段重渲(Design 第二轮"稍微改了一下",2026-06-20)**:Design 把逐字浮现从 **JS 定时器逐个加 `.on`** 改成 **CSS 动画驱动**(每字 `animation-delay` 累加 + `@keyframes mfx-char` blur(4px)→清晰,走合成器时间线,**不受 JS 定时器节流影响**)——正是冲着"整段重渲冲突"来的,等于说"现在可以接了"。落地 `_streamWrap(el, shownN)`:每个 `answer` delta 重渲后,把正文文本节点按 `字/词` 切片包成 `.mfx-w`;**下标 < shownN 的字(上轮已显示)打 `.mfx-shown`**(CSS 覆盖成 `animation:none;opacity:1` → 即时不重播),**下标 ≥ shownN 的新尾巴累加 `animation-delay` 错峰淡入**,光标插在最后一个字后;返回总字数作下轮 `shownN`(`_revealN`)。前缀稳定性已验证(累加 delta 下 token 序列单调增、前缀不变,故 prefix-skip 对齐成立)。两道保护:长答案 `>5000` 字跳过逐字(只留提亮+光标),markdown 标记(`**`/`` ` ``)中途闭合会让少量字重播一次(可接受)。`.mfx-w` 只在流式期存在,收尾 `renderMd(...,true)` 重渲成干净 markdown+MathJax(无 span)。
