...
**图描述管线方向(2026-06-20,设计定调)**:用户要把整页 AI 描述改成「裁图描述」省 token(实测裁图比整页省 **74%**、质量还更好——只要 prompt 明说『这是从书里截出来的图、描述它的内容』,A/B 测 V 模型裁图版反而点出了『绿色箭头构成 V 形』等视觉细节)。架构定调:**YOLO 当 gate**——`figures_geom` 框图:**0 框→跳过(无图,零 AI)/ 1 框→裁那张图+文字+provenance 描述 / ≥2 框→整页描述**(多图保版面关系,不拼裁图)。YOLO 召回调低 conf 拉高(`detect_page(pg,conf)`:応用情報 conf 0.2→0.05 框数 页5 2→5、页31 1→3)。⚠ **YOLO 在 Pi CPU ~6.7s/页**(实测)→ 不能即时跑,改 **闲时 systemd 批处理**(`yolo-figures.timer` 每 6h,Nice=19)。provenance 来源 = §「书籍目录」功能建的 TOC → `_book_location(印刷页)`。**已落地**:目录功能、provenance 注入 describe(`describe_page_figures(...,location=)`)、YOLO 闲时 timer(`yolo-figures.timer`)。
**剩余 = 一个 load-bearing 反转(下一步,当专注单元做)**:现状「AI 页级先描述 → `yolo_figures.py` 事后精修框」,要倒成「YOLO 先 gate → 夜间裁图描述」。三件一起改:
① **`yolo_figures.py` 给所有页记图框**——现在只在 `fig_pages = set(by_page)`(AI 已描述的页)上记 YOLO 框,其余页只记公式 → `figures_geom` 没有「AI 没描述过的页」的框,gate 无从谈起。改:`detect_page` 本就每页跑了,把每页的 `det["fig"]` 也存进 geom(0 框页也记 none)。
② **新建夜间描述批处理**(用户:截图 AI 描述用夜间凌晨 off-peak 额度跑):读 geom 框,**0→跳过(零 AI)/ 1→裁图+文字+provenance 描述 / ≥2→整页+文字+provenance**(裁图描述 prompt 要明说『这是从书里截出来的图、描述它的内容』,实测比整页质量更好)。幂等,挂凌晨 daily 窗口。
③ **`/api/page-figures` 改只读**——白天阅读只读现成描述,不再 `_fig_describe_bg` 触发 AI。
依赖:必须等 ① 的 YOLO 批处理先跑完一轮填好 geom,② 消费的框才完整。影响面:图徽标/全文搜索/助手都吃图描述,反转要小心(先加新批处理跑通、再切只读、最后退役旧 lazy describe)。

**装饰性卡通吉祥物被当内容图描述(2026-06-20)**:用户「这种图像也被 ocr 了」——応用情報 里戴眼镜讲解小人(版式装饰)被 `describe_figures` 当内容图描述,还(经 §39「读当前页带图描述」)灌进助手上下文。用户提议「描述前在库里找有没有类似的再决定」。**试了感知哈希(dHash)跨页去重:没用**——25 图聚成 24 桶、无 ≥3 页反复桶,因为这些吉祥物**不是像素级相同的同一张图**(不同章节小人姿势/上下文不同 + AI bbox 近似),是**语义上**都属装饰角色而非像素相似。结论:**语义判断才对路** → 改 `describe_figures.PROMPT` 的跳过清单,明确「反复出现的卡通吉祥物/角色插画/讲解小人/页眉页脚装饰图标」一律当'没有插图'不描述(跟已有的封面/logo/条码 skip 同列)。感知哈希那套机器实测无效已撤掉(避免无用复杂度+每图多一次渲染);存量 sidecar 里那张 p35 吉祥物手动删除 + 该页标 `_none_pages`。**教训:'反复出现的装饰元素'多是语义类别(同型不同图),用 AI 语义判,别上像素 hash。**

**bug:去边模式整页不出徽标(2026-06-20 修,`reader-figcrop-96`)**:用户「开了插图描述却没徽标」,自测**关掉去边就出现**。根因:模板里 `.char-layer/.vocab-layer/.hl-layer/.ruby-layer` 各有 `position:absolute;inset:0`,**唯独 `.fig-layer` 没有任何定位规则**(JS `ensurePageLayer` 给的 `fig-layer page-layer` 类两边都没 CSS)。非去边时 badge 碰巧锚到 `.page-wrap`(relative)能用;**一开去边,`.crop-on>*{transform:translate(...)}` 让 fig-layer 成了定位上下文,但它是静态块(无 inset:0)→ badge 锚点跑飞、被 `.crop-on{overflow:hidden}` 裁没**=整页无徽标(费曼没开去边故正常)。修:① 模板补 `.fig-layer{position:absolute;inset:0;z-index:9;pointer-events:none}`(它带 `page-layer` 类 → 去边时也吃 §36 那条 `--full-w/h` 撑满整图);② `draw()` 去边时 cssW/cssH **改用 `--full-w/--full-h`**(fig-layer 被撑成整图,徽标归一坐标也是整图基准,不能再用裁后的 `clientWidth`)。**新增叠层务必同时给 `.xxx-layer` 加 absolute+inset:0**。

**bug:YOLO 几何层遮蔽 AI 图层,漏检页整页不出徽标(2026-06-20 修)**:`/api/page-figures` 原来「`figures_geom`(DocLayout-YOLO)存在就全用它,否则才回退 AI `figures`」。但 **YOLO 召回率比 AI describe 低**(実測 応用情報:YOLO 15 页 vs AI 22 页,漏了 5/37/41/45/46/47/49),路由只认 geom → **这些页 AI 已描述了插图也整页不出徽标**=「开了插图描述却时灵时不灵」。修法:**per-page 回退**——`geom` 有这页就用 geom(精确 fbox/去重/组),`geom` 没这页就回退该页的 AI `figures`(漏检页也出图);`geom` 整个为 None(没跑 YOLO)才全用 AI。验证:31 仍走 geom(V 模型),41/37/45 回退出 AI 图,27(`_none_pages` 确无图)仍空。
...
- `owner=pi` 只保留显式远端备份/同步、用户选择的远程预处理、外部系统写入，以及由 Pi 作为自建
  AI API 提供的 CLI/模型能力；本机可完成的设置、书籍数据和用户修改不得归入此类。按书远端请求
  必须把不透明本机 `localBookID` 核验并改写为已绑定的 Pi 书籍身份；没有绑定时失败可见，禁止
  串书或假成功。
...
### 单击纯汉字词的日语判定（2026-09-07 用户实锤「2位 心疾患」）

用户：「有些词可以滑动选中但是无法单击选中」。单击链路（`reader.src/13-selection.js` onEnd 单击分支）对**未声明语言**的书，
纯汉字词只看所在句有没有假名（`hasKanji(_t) && hasKana(_ctx)`）；列表/标题/表格里的孤立词整句就一行（「2位 心疾患」）、
没假名 → `isNativeHan` 成立 → 当成母语中文词、连选中高亮一起清掉，表现为"单击没反应"。拖选走别的分支所以正常。
...
### iOS 键盘把整页顶上去，底下露黑边（2026-09-11）

点侧栏输入框 → 整个画面向上抬起，底部留下一条黑带。

成因是 iOS 的做法：软键盘弹出时**不缩小 layout viewport**，而是把整页往上推
（`visualViewport.offsetTop > 0`）。页面自己没有可滚区域时，被推走那块下面
露出来的就是 body 底色。

修法两件事一起做，缺一条都还会露：

1. 把页面按回原位 —— `window.scrollTo(0,0)` **和**
   `document.scrollingElement.scrollTop = 0` 都要（iOS 上两者不总是同一个）；
2. 用 `visualViewport` 量出键盘挡住多高，写进 `--rc-kb`，垫在
   `#side-pane-asst.active` 的 `padding-bottom` —— 输入框靠**面板让位**浮到
   键盘之上，而不是靠"把整页推上去"。

⚠ 只在输入框真的有焦点时生效，失焦立刻撤掉：键盘高度是个临时状态，
留着会让面板永远短一截。没有 `visualViewport` 的浏览器一概不动。

⚠ 键盘是**动画**弹出的，量一次不够 —— 60/220/500ms 各量一次，
另外挂 `visualViewport` 的 resize/scroll。

⚠ **助手面板有两份活的实现**：`rc-assistant.js` 与
`reader.src/25-assistant.js`（拼进 `reader.js`）。只修一处的表现是
"某些书里黑边没了、另一些还在"，而那种局部好转最难被认成"还没修完"。
