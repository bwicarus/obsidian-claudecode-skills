...
- **F6 词组面板加载期呼吸 + 高亮持久**：`showPhrasePopover` fetch 期间给原选中 `.sel-overlay` 加 `phrase-breathe`，`finally` 里**用 live DOM 查 `.sel-overlay.phrase-breathe` 移除**（不能存开始时的元素引用，中途可能被别处 pointerdown 清掉变 stale）→ 转常亮。**点别处只藏面板、保留选中高亮**（用户要求「点击其他内容时这个词的高亮不消失」）——word-pop 的 pointerdown-outside 不再清选中；高亮只在重新选词 / 点空白（onStart 清）时自然替换或清除。
