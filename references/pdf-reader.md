...
**注入位置对齐原字形**(2026-06-19 追加,`_OCRFIX_INJECT_VER`=6):一开始把校正文字**均匀平铺**满 bbox,选中框跟图上的字「很大偏差」(用户反馈)。根因:OCR 给文字不给位置,而 LaTeX 源码(`$1\times10^{-8}$` 16 字)远长于其视觉宽度(`1×10⁻⁸` ~6 字形),均匀铺会把数学段撑过宽。**修法**:删原坏字符前先**捕获它们的 bbox**——坏文字层只是 ToUnicode 把字符『值』错了,『位置』是对的(PDF 渲染就靠它)→ 把校正文字按**累积视觉权重**比例插值到原字形真实 x 边界上(`_ocr_visual_weights`:`$^_{}`、命令名字母权重 0、`\`命令≈0.8、空格 0.3、可见字 1 → LaTeX 标记被压扁不占宽)。**两个隐藏坑**:(a) 原字形排序不能按 `(round(y0/4),x0)`——上标 `⁻⁸` 的 y0 抬高会被分到别的 y 桶,基线字全排一组、上标全排一组 → 整体 x 不单调、定位全乱;改**按行聚类**(y 中心差 >7pt 才换行,上标 <7 算同行)+ 行内按 x。(b) 改注入逻辑要 bump `_OCRFIX_INJECT_VER` 否则 cv 不变、旧缓存不刷。验证:费曼 p24 校正各 token x 单调不重叠、数学段按视觉宽度压缩、阅读序正确。

**边界**:① 被丢的 `Å` 这类符号要被收进校正,拖选范围须**带过它的位置**(选到后面字),否则裁图不含它。② OCR 仍可能错(如边缘吃字"金为");靠"你看过结果才存"+ 重 OCR 覆盖 + ⚙ 切 opus 兜底,不焊死。③ 位置对齐靠原字形真实坐标,单行准;多行选区的 bbox 是大矩形、跨行 x 会重置(edges 非单调),对齐退化为近似——单行选最干净。④ 复制/翻译用途(整段文字)永远对,不受位置近似影响。
