...
**日英 mastery 统一（2026-06）**：日语生词原来是 looks 计数二元上色（≤1橙/≤3黄/≥4绿 + mastered），跟英语的分数模型不一致。改：`_jp_mastery(e)` 算 0-1 分数，**跟英语 `compute_score` 同一套信号哲学**——用户锁(`user_mark`/legacy `mastered`→1.0/0.0) > 查询次数(looks≥2/3/5 各 -0.05/-0.15/-0.25) > 时间衰减(last_ts >30d +0.05 / >90d +0.10)；`_mastery_slug(m)` 用**跟英语 `compute_mastery.LABELS` 完全相同的阈值**(`<0.25 new / <0.55 seen / <0.85 known / else mastered`) → 下划线颜色两语言完全一致；mastered 不画（同英语跳过 `label_slug=='mastered'`）。**冷却**：刚查过 `last_ts≈now`→days=0→无回升加成 → 24h 内只会因再查降、不自己涨（=英语「查后只跌不涨」的等价行为）。`/api/jp-vocab-mark` 改写 `user_mark`(known/unknown)跟英语 `/api/vocab-mark` 同口径；单词小框掌握 toggle 日英共用 `_wordPopMaster`（统一发 known/unknown）。⚠ **未做**：日语暂无英语那套「Anki 卡链接 + 段落暴露(`paragraph_exposure`)」信号，也没并入 vault `资源/vocab/` 笔记库（那是更深的单库统一，需新写日语笔记生成 + 暴露计算 + 迁移 jp-vocab.json，风险高暂缓）；故日语 mastery = 查询次数 + 时间衰减 + 用户锁，比英语少「被动暴露提分」一项。
