...
> ⚠ **2026-08-02 时点交接（当时安装 0.1.45），勿据其推断现状**：标题与 §0 的可用性表都已过期 —— Classic 上行在 2026-08-03 `a14e7e2b`「修通 GPT Classic 电脑客户端语音冷启动与启停」后打通，同日 `ios-app-extension-merge-handoff-20260803.md` §5 记「两个目标均已实测可用，已安装 0.1.67」；§2 剩余的 A/B 两项所在的语音启停链路又在 2026-08-17/18 被整体重做（`cd2f6081`「重做「打开与保持」」、`c7c1c02f`、`408c8be3`），桥候选现为 0.1.159。§1① 的 `C:\iCloudDrive` 死副本坑也随 Swift 源码进仓库（`ios/BWReader/App/`）而消失。**但 §4「血的教训」四条（搜索先用正例校准 / 日志先比时间戳 / 诊断不能改变被诊断系统 / 共享脏工作区不可信）至今仍然成立，可继续引用。**
