...
**YOLO 已在 Windows**：PC 预处理 worker（`scripts/reader_pc_preprocess_worker.py`）每页本来就过一遍 DocLayout-YOLO（GPU）只留公式框；2026-09-07 起同一趟把 figure/table 框也留下（payload.figures），服务端 `upload_pc_formulas` 校验后落 `pc-figures.json`，`complete_pc_worker` 发布成功后 `_mirror_pc_layout_to_pdf_figures` 镜像进 `state/pdf-figures/<sha1(路径)[:16]>.json`（formulas 含 LaTeX、figures_geom yolo 口径、保留同页 IoU≥0.7 的旧 desc）。Flask 阅读器公式层与 KJ 页级分析的 boxes 都读这份边车。⚠ Pi 时代的边车键是 Pi 路径的 sha，Windows 上对不上（费恩曼 `1eadda6a2a33cb4d` 就是孤儿）；重跑一次 PC 预处理即可重建。
**整本节点析出（手动按钮）**：App 书库页「文字、分词与公式」面板里、PC 预处理旁一行「整本节点析出」（`ReaderLocalLibraryView.kjScanRow`，协调器 `ReaderPiOCRCoordinator.kjScans`，客户端 `ReaderPiOCRClient.kjScanStart/Status/Cancel`）→ Windows Flask `POST/GET /pdf/api/library/kj-scan/{start,status,cancel}`（挂在 /pdf 下是因为 tailscale serve 不转 /kj）→ detached 低优先级跑 `scripts/kj/book_scan.py`：滚动会话（按目录一级条目分窗口，没目录 25 页一段；窗口累计输入超 `--window-tokens` 换会话，开场接力包由账本生成）、逐页 页图（长边 1568）+ 阅读器页文字接口（Bearer 令牌，兜底 PyMuPDF）+ 边车 YOLO 框 → 固定 JSON 结构 → `pages.submit`；Claude CLI（stream-json 同进程多轮）或 Codex CLI（`exec --json` / `exec resume`），auto 时 Claude 未登录自动切 Codex；状态 `state/kj/scans/<sha 前 16>.json`（每页 usage：input / cached / output，即『每页真实 token』），`<status>.cancel` 取消，重跑跳过已分析页（`--force` 重做），连续 3 页失败停。整本自动化仍未开：等 token 数字看过再说。
