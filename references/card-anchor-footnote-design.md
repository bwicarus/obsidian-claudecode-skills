...
> 状态：**已实现，待 2026-08-20 发布后真机验收**。与用户逐条讨论敲定，全部前提已对着代码核实过
...
**词区间锚 + 分类色框 + 右侧悬浮浮标轨**；手动钉和 AI 自动钉产出**同一种标记**；
...
| 部件 | 做法 |
|---|---|
| 词框 | 透明底、分类色细描边；左上角折线与底部断续线作轻装饰，不盖字 |
| 序号 | 骑在词的**右上角**，落进行间空隙；**无实心底**，用 `text-shadow` 打纸色光晕 |
| 点击 | 词框本身 `pointer-events:auto` → 展开卡片 |

词框保持透明底，因此刷新、缩放后重画也不会改变正文像素的对比度。
...
- `body` 下的 fixed 悬浮层，覆盖正文右侧 150px，**不占布局、不新增独立栏**
- 色块背景用颜色混合得到半透明表面；数字本身保持实色高对比，不能继承整块 `opacity`
...
卡片只从当前书籍 App-owned `document-notes-legacy` 权威记录读取；`card-placements`
与 `entity-references` 是同一批事务生成的投影。页面 DOM、短命词框和右侧浮标都不是 AI 数据源。
删除事务成功后发 `reader-local-notes-changed/1`，当前页上下文立即按几何顺序重排；失败则保留旧上下文。
...
```
…タコス⟦CARD_START n="6" id="…" type="card" label="背景卡"⟧内容…⟦CARD_END⟧、エンチラーダ…
```
...
3. ✅ 标记层（`.pgbind-layer` + `.pgmark` + `.pgmark-n`）和 fixed 右侧浮标轨
...
7. ✅ `buildLocalPageContext` 的成对 `⟦CARD_START…⟧…⟦CARD_END⟧` 内联标记；
   只读权威存储，删除后即时重建，并保证 12k 截断不留下半个卡片块
8. ✅ `bind` 字段白名单（跑 `python scripts/contract_sites.py card-top-fields`）
9. ✅ C# `CopyEmbeds` 的 `_reason` 闭集

### 变更过的决定

- **浮标轨已恢复**：它浮在正文右侧而不是独立栏；正文词框和轨道浮标都是同一卡片的快捷入口。
  展开卡复用正式 `.vc-card`，直接 portal 到 `body` 的 fixed 顶层并按视口四边收束，页边和页底不裁切。
- 词锚展开态不再显示关闭叉：右上角是明确的垃圾桶删除；只有权威删除墓碑成功后才撤卡、词框和浮标。
