...
## 批次 105(2026-07-13)笔迹清空静默注入增强 + 缓存删除语义确认

- **笔迹全擦掉**(用户设计):RTC 版 ink 清空注入原已存在但文案弱("清空了")——增强为「已擦掉全部笔迹,**不要再调 see_ink**(看了也是空白),再提笔迹直接说已擦掉;直到下一条笔迹更新状态出现前一直成立」。空 strokes 的指纹路径(except→`{ip}:0`)不会被去重拦截✓。WS 版 ink 本来就不注入(只存状态供 see_ink ctx),保持对称不动。
- **item.delete 与缓存**(用户问,确认其理解正确):前缀缓存从对话头逐字匹配——删除点**之前**照常命中,**之后**全部 miss 一次(下轮起重新形成缓存)。焚图净收益=被删图的每轮重复 token(几千/张,cached 价也不免费)>> 删除点后缀重付一次(最近一两轮文字几百 token);且策略上删除动作越少越集中(新话轮批量焚上上轮及更早)缓存断点越少。#285 全历史压缩同原则:头部一次性大动作,不频繁小删。

## 批次 106-109(2026-07-13)收藏夹苏醒/笔按钮退役/字幕定位/start单飞锁/Grok静默停推

- **106**:收藏夹按钮开页主动拉一次列表(懒加载→有存货不显示的根因);`#ink-fab` 圆形笔按钮 CSS 退役(手写收敛顶栏)。
- **107**:收起侧栏开语音字幕"消失"根因=`_capPlace` 避让算式——通话条内嵌侧栏(vc-inline)被收起时 rect 为 0,b=屏高+10 把字幕抬出屏幕顶端;避让改仅浮层真实可见(height>0 且 top 在屏内)时生效。
...
## 批次 119(2026-07-13)打断失效根修:半双工条件化

用户实测 Grok 无法打断——根因=relay 对 WS 引擎默认下发 `half_duplex:true`(㉙外放防回声成熟解),前端 onCap 在 **AI 播放期整段丢弃麦克风数据**:说话根本到不了服务端,111/116 的打断链(cancel/450/pend_resp)全部空转。修:丢弃条件收窄 `_halfDuplex && !_hpIn && !_abridge.on`——**耳机在线或回声桥连通=全双工可随时打断**(耳机无回声;桥=浏览器 AEC 有远端音轨参考);仅"外放+无桥"保留半双工兜底(否则模型自听)。`_hpIn` 通话建立时检测+`devicechange` 插拔实时更新。注入原则同批记录(用户):**收益驱动的大块注入**,不是习惯性堆料——118 的页正文属高价值;低价值内容别塞。

## 批次 119b(2026-07-13)"工具后行为怪异"根修:出飞计数顺序 bug
