...
1. **EPUB 的其余部分。** ⚠ 先想清楚「原生化 EPUB」是什么：EPUB 正文是 XHTML，
   ...
   ⚠ **点已有划线去编辑不是缺口**：EPUB 早就有，走共享层 `RC.highlight.openEditor`
   （改色/备注/删除的回调 EPUB 都接了），而且它的网页层是可见的 —— 能用。
   与 PDF 的差别只是"网页浮层 vs 原生面板"。（2026-09-22 我一度把它记成待办，
   查了代码才发现是现成的。**EPUB 这侧判断"缺不缺"之前先跑一遍**：它不像 PDF
   那样有一整层被藏起来，很多东西其实还活着。）
   真正还没做的只有：叠加层不统一（生词下划线/振假名等仍由网页画 —— 同样是
   **没有坏**，只是观感与 PDF 两样）。
