...
- 2026-09-21 原生导航续接：目录使用 `RC.readerTOC` 共用原始目录与跳转，SwiftUI 展示
  章节层级、过滤、刷新与连续选章。PDF 保留印刷页换算及返回锚，EPUB 保留原节定位。
  页码改用 `RC.readerNavigation` + 原生弹出面板，滑动松手才跳页，保留输入页码、跨卷、
  前后翻页和返回；不再伪造 page-scrub 指针事件。换书/旧目录/加载失败都有失败回执。
  八项离线 Chromium 检查中七项通过；旧测试改为“只有网页目录按钮不算原生能力”后，
  失败项定向复查通过。网络审计 0 新债务。`bc43a261` 在 `35561109116` 通过
  Apple 模拟器编译，签名/归档/上传跳过。随后 `201f2534` 补 App 命令白名单四项；
  目录/导航两项浏览器验证加入 Swift 白名单检查后通过，未为常量名单单独重复构建。
  用户允许开发中直接采用更好的实现，但原数据/功能/操作链必须保留。

- 2026-09-21 继续原生书页卡片：普通文本/学习卡的书页展示与卡头拖动、收放、正文锚定、
  移除接入 SwiftUI；共享 placement 仓库和学习卡状态仍是唯一所有者。放置等待保存回执，
  浮动卡转移成功后才关闭原浮动卡；旧版本操作拒绝执行。关闭侧栏后投射原有浮动卡队列，
  保留原定时收纳/关闭和阅读续时规则，不从聊天历史额外复制一批卡。
  定向 Node 37 项、全量 Node 2112 项通过；六项 Chromium 检查中旧拖放 fixture 更新为
  异步保存回执后，失败项及新书页卡片用例复查通过。未做真实 iPad 手势/Anki 写入。
  墨迹/复杂媒体书页卡片仍待完整原生迁移，不计入完整性验收。
  Actions 新增 compile_only：本批搜索与书页卡片合并检查 Swift，跳过签名归档/上传。
  `8ff8e8cd` 在 `35559373095` 仅编译通过（5m2s），确认未执行签名/归档/上传。
  续接已把原生卡层移到 PencilKit 之下，并加换书/账号隔离；原生词锚描边与编号、
  点击展开/收起复用原开关逻辑。已锚卡拖到空白后保持可见的自由卡，修复隐藏残留。
  原生绑定/浮动转移两项浏览器用例通过，绑定相关定向测试通过；`dd3aaece` 在
  `35560090299` 通过模拟器编译（3m14s），签名/归档/上传均跳过。
  没有更新 TestFlight 或重启服务。
...
- 2026-09-21 完全原生化开发续接于 `codex/reader-fully-native-20260921`：
  ...
  `f3c4056e` 后在 `35548259486`（768）完成同样的编译/归档/导出，未上传。
  ...
  三项桥接浏览器用例及 2104 项 Node 合同通过。
  `494a395a` 在 `35549846587`（769）构建成功；电脑通话目标/状态/错误原生页
  `4c448adc` 在 `35550229924` 同样通过模拟器编译、签名归档、校验和 IPA 导出。
  两次都未上传 TestFlight。继续接通原生复习队列、撤回评分、回答/段落选择、草稿确认等；
  2110 项 Node 合同与 Chromium 原生复习定向用例通过；`dfc1515c` 在 `35551431246`
  通过模拟器编译、签名归档、校验及 IPA 导出（11m1s），未上传 TestFlight。
  全文搜索已接原生结果列表，沿用本机 PDF/OCR、EPUB 查询与定位高亮；定向 Chromium
  检查通过，覆盖旧查询取消、过期结果及换书拒绝。此批 Swift 尚未 Apple 编译。
  用户明确要求相关功能合并编译，禁止每做完一个功能就跑签名归档；交付前集中验证。
  阅读区、其余设置和列明的旧 UI 依赖仍需迁移，未宣称整版完成。
...
## Codex：Reader 工具、媒体、历史与电脑语音断连收口（2026-08-13 JST）
- **改了什么**：图片/视频卡走安全代理，历史只发布最终回答；新增精确高亮与 Anki 草稿工具；ReaderPC 复用唯一 broker；App/扩展断线后立即归还电脑音频路由，异常时 30 秒仅重试一次。
...
- **边界/下一步**：词典不进 IPA/书附件/Pi，查词不自动使用 ReaderPC；Direct `0.1.121` 已按原子流程安装且清单现场可核验，Pi/桌面正式 channel 待真机验收。
...
## Codex：原位卡片快照与漫画文字层几何正式发布（2026-08-22 JST）
- **修复**：提交 `59918a08` 保留 MangaPageOcr 分框/分行/方向，只在既有行内按真实墨迹对齐字符并在低置信度时回退；Direct `0.1.177` 将卡片元数据与完整正文留在锚词后的成对 marker 内。
- **缓存/验证**：Pi `pi-default-v2`、PC `quality-first-v3` 拒绝复用旧页缓存，App 以 `*-manga/2` 刷新几何；真实第 28 页“美味しい”四边相对 Vision 均小于 1 页面单位，专项 Python 与 Reader Node 全量 `1654/1654` 通过。
...
### 2026-08-22 Vision 原文 + 结构化 Markdown 快照正式发布（0.2.135 / ReaderPC 0.1.58 / App 1.1.81）
...
### 2026-08-23 跨页锚定与 Direct 离线可靠投递正式发布（0.2.137 / Direct 0.1.178 / App 1.1.82）
- **修复**：提交 `bef7abd9` 支持同书离屏页原始字符坐标锚定；Direct 在 App 后台、非当前页或来源短暂离线时持久排队并幂等重放，自由卡绑定复用稳定 ID 且仅对网络/5xx 重试一次。
- **一致性/验证**：结构化快照同时保留 Markdown 阅读流与 `pageChars`，锚定卡使用确定性 `noteId/mutationId`；Reader Node `1704/1704`、Direct Release/完整自检、网络审计及 Pi 全门禁/E2E 通过，网页便签完整矩阵复跑通过。
...
- **验证**：本地阅读启动、原生偏好与接口兼容合同 17 项通过；独立离线 Chromium 集成覆盖真实 turnCard 投影、语音发送分支、卡片原件定位和生命周期。handoff 零错误，197 份 Reader 合同文件及现有发布门禁通过。
- **发布**：源码 `e258c30c547222f190e9e9a1c1787be7f0f630a8`；Actions `35532547315` 通过模拟器编译、设备归档、签名校验并上传 `1.1.83 (757)`；状态查询 `35533199560` 确认 `COMPLETE / VALID / IN_BETA_TESTING`，零错误警告。外部测试状态为 `READY_FOR_BETA_SUBMISSION`，本轮没有提交外部审核。纯 App 发布，没有数据库迁移或服务重启。
- **尚未实机验证**：iPad Apple Pencil、横竖屏/键盘交互、真实语音及完整 Anki 导出；不能把原件兼容入口称为全功能原生重写。PDF/EPUB 文档引擎和 Safari 扩展继续使用现有实现。

## Codex：纠正原生侧栏功能回归（2026-09-21 JST）
- **用户实测**：1.1.83 的原生预览不具备原卡片的拖放/锁定、Anki 状态机和输入区选区附件；原生侧栏关闭时旧 drawer 仍被强制保持 open，导致生成物无法按旧规则显示到阅读区。上一轮“有原件入口”不能视为功能完整。
- **修复边界**：保留原生书库/导航/设置；会话、生成物、Anki 和选区附件重新使用与书页同一 WKWebView 内的完整共享组件。右上角按钮直接调用真实 drawer 的 open/close，桥只读取它的状态，不再隐藏或强制打开旧侧栏。没有新建第二份卡片/历史库、没有数据迁移。
- **迁移约束**：禁止再次用 SwiftUI 卡片预览替代活卡；原生替代方案必须先覆盖跨侧栏与正文的拖放、原位置锁定、关栏投递、Anki 编辑/确认/学习/导出和选区附件链路。完整性优先于渲染技术。
- **验证**：离线 Chromium 使用真实 rc-sidedrawer/rc-voicecall/rc-flashcard 与原 PDF 焦点组件，验证卡片拖出仅产生一次同 gid 的完整落点请求、关闭后 renderInfoCard 在阅读区可见、焦点 chip 可见且可移除。持久化边界使用内存回执；未对用户数据写入，iPad 触屏与实际持久化仍须实机验收。候选版本 1.1.84，发布回执待补。
- **发布回执**：源码 `ff5e2d6f1283ee70b78467de3af45057cb28d541`；构建 `35542239383` 通过模拟器编译、签名归档与上传；查询 `35542922909` 确认 `1.1.84 (760)` 为 `COMPLETE / VALID / IN_BETA_TESTING`，零错误警告。外部状态 `READY_FOR_BETA_SUBMISSION`，未提交外部审核。本轮 handoff 零错误（197 份合同文件），原生启动/偏好 10 项与两项离线浏览器集成通过。保留原始回退 tag `reader-native-baseline-20260921-9078aecf` / `1.1.82 (753)`。

## Codex：继续原生侧栏与单行可折叠顶栏（2026-09-21 JST）
- **实现**：原生顶栏统一书库/书名/页码/阅读工具/AI 入口，40pt 单行可收起成悬浮把手；旧 PDF/EPUB 顶栏操作投影到原生菜单，调用原处理器。恢复 SwiftUI 侧栏，开闭和共享 drawer 同一事实源，关闭后仍按旧规则把生成物送到阅读区。
- **交互**：原生选区提示接原选择/清除；Anki 从实时实体读取正背面、编辑字段和保存/评分/导出按钮，操作回原状态机。卡头系统拖放通过 WKWebView 坐标转换调用原 placement，保留 gid/cid；拒绝过期 scope 和正文外落点，不新建历史或学习状态库。
- **验证**：离线真实共享组件通过：流式/隔离、选区、编辑、答案显隐/四级评分按钮、原生及旧式拖放、关栏投递；17 项原生导航/打包接口合同通过。拖放持久化端在离线用回执替身，实机 Pencil、跨栏手势及重开书后的落位需用户验收。
- **范围**：复杂 HTML/媒体、原复习队列及完整流程仍保留原件入口并可返回原生助手，不能称全部功能已纯原生。候选 1.1.85；1.1.84/f41ed8a0 和迁移前 tag 保留，无数据库变更、无服务重启。
- **发布**：源码 `0f3f9bfd2c8fa6f0f8bb9b581430bd9e9ba22cc6`；构建 `35543975399` 通过模拟器/签名归档/上传，程序化等待后的状态查询 `35544636126` 确认 `1.1.85 (764)` 为 `VALID / IN_BETA_TESTING`，外部仍 `READY_FOR_BETA_SUBMISSION`。未完成的早期构建 `35543873620` 已取消，未上传。

## Codex：完整原生化范围确认与会话展示接口（2026-09-21 JST）
- **最新用户要求**：原生能力实现原版全部功能；允许统一样式，禁止删减逻辑或以切回原界面代替原生实现。旧 HTML/JS 数据保留，逐类迁移，暂未迁移明确标记。完整约束与任务链见 `reader-native-migration.md`。
- **范围**：独立分支 `codex/reader-fully-native-20260921`；本次只完成会话展示的语义快照，原生结构化轮次不再读取网页气泡文字。持久化仍排除草稿，快照深拷贝保留原始生成物与实体身份。
- **验证**：展示/去重合同 11 项、离线桥接 2 项及 Reader Node 合同全量通过，共享 vendor 已生成；包含网页文字损坏不影响原生流式原文、草稿不提前入库、快照不能改写原件。没有宣称整条会话/卡片链已摆脱网页运行时。
- **状态**：未发布、无版本号递增、无数据迁移与服务重启；已发布 1.1.85 不受影响，原回退 tag 保留。原生阅读区、卡片与复习等完整链路继续待迁移。当前会话无 BWAB_CLI，未使用已退役邮箱或发送跨 agent 消息。

## Codex：原生卡片操作解除网页按钮依赖（2026-09-21 JST）
- **实现**：同一迁移分支新增 `interactionState/performInteraction`，原生编辑、保存、删除、翻面、评分与导出直接使用卡片语义接口，不再模拟隐藏网页按钮和输入框。网页投影和原生控件共享编辑/翻面/受控评分处理器，保留原仓库与状态机。
- **回执**：原生编辑等待持久化，确认/删除等待仓库完成；结果未知仍保持防重复。评分/外部导出只是发起操作，其完成状态由原管线回执决定。新接口没有另建实体或学习状态。
- **验证**：离线真实组件及卡仓（内存后端）2 项通过，覆盖移除网页按钮后编辑/翻面仍可用、确认/删除落库、稳定索引、受控评分回调/重复拦截，以及已有选区和拖放链。Reader Node 全量 `2104/2104`；vendor 已重建。
- **边界**：卡面正文、placement、阅读区仍有旧运行时依赖，尚未达到完整原生化；无 TestFlight 上传、真实外部 Anki 写入、数据迁移或服务重启，未验收实机手势。

## Codex：继续原生卡面、调用详情与选区链（2026-09-21 JST，开发中）
- **方向**：用户纠正开发节奏，要求在当前任务中持续推进。误建的 `reader-2` 本线程定时续办已删除；不再依赖定时任务接续开发。
- **实现**：卡面正背面/状态改为结构化数据；SwiftSoup 2.13.9 解析原 HTML，TextKit 原生显示标准富文本和 ruby 标注、提供系统文本选择与链接。选区接入原焦点上下文及保持/释放 TTL。工具参数、结果、错误与步骤按需进入原生详情；卡片“完整网页界面”入口改为原生资料面板。
- **保留**：不执行旧 HTML 脚本，内嵌媒体和交互明确标记未迁移且保留原件；完整功能尚未迁移完毕。旧设置、复习队列、placement 和阅读区继续待做；无用户数据迁移、无服务重启、不上传 TestFlight。
- **验证进度**：离线真实组件/卡仓测试通过，包括卡面数据、读取工具详情不切换侧栏、旧 scope 拒绝，以及原生选区保持/释放。共享合同全量和 Apple 候选编译待收口；注音跨行、真实 iPad 选择手柄尚未验收。

### 2026-09-24 Jev 最新生成物重发
- 共用输出入口增加四秒去重、最新 Reader 卡/Anki 草稿记录和本机重发接口；重显不新增原文放置，草稿沿用 draftId。
- 语音侧收到 Jev 重发选择即启动程序并发送尝试通知，成功静默、失败另报；同轮重复通知共用同一任务。
- Python 注入/重发检查与原生发送自检通过；候选 0.1.437 已构建，安装回执见本机 voice-cli/experiments/jev-live/resend-deployment.json。
- 不变更 iOS 安装包或 Pi 服务；真实语音重发显示仍待用户验收。

### 2026-09-24 Codex：App 完整原生迁移（隔离候选，仍在实施）
- 分支 `codex/native-pdf-runtime-20260924`：PDF 出图/文字定位、批注手写事务、续读、同步待发队列已移至 Swift；对话传输改增量并停止隐藏文本渲染。
- 卡仓命令由原生事务执行并保持旧实体/状态/回执合同；Anki 卡面停止网页 HTML/图片排版，Swift 接管显示状态，其他网页业务依赖继续迁移。
- 验证：Reader 聚焦合同与 Apple workflow 内纯 Swift/浏览器对照；最新候选完整编译状态以该分支 Actions 为准。
- 尚未上传 TestFlight、安装新 App 或声称完成迁移；实机手写、多设备同步、内存与电量仍待验收。后续继续原生卡片交互、消息输入及剩余旧运行时替换。

### 2026-09-24 Codex：原生 Anki 导出及打包入口同步
- AnkiMobile 与 ReaderPC 导出移至 Swift 持久化回执和独立数据连接；每张卡的知识节点归属保留，普通卡翻面直接更新原生仓。
- 修正 `reader.js` 生成物未同步的问题，App 定位入口与源码一致，只取原生字符数据；vendor 已重建。
- 验证：Reader Node 全量通过；前一阶段完整 Apple 编译通过，本批 Swift 变动继续由候选分支 Actions 验证。
- 仍未发布或安装；复习控制器、对话接收与其他旧业务依赖继续迁移，不能视为完整原生版。

### 2026-09-24 Codex：原生词典与复习提交（迁移继续）
- 本机日语词典检索、活用与兼容字段转至 Swift；字典缓存有上限，网页不再载入本机字典分片。Apple 完整编译及真实词典对照通过。
- 复习队列由原生卡库读取；评分的调度、历史和状态同事务提交，保留原调度规则、稳定编号及修订校验；成功后才投递外部学习事件。
- 验证：Reader 评分/撤销与原生桥合同通过；新增 Swift 评分对照和失败回滚由候选分支 Actions 验证。
- 未发布或安装 App；复习控制器、账户事件投递、对话接收与其他旧业务依赖继续迁移，实机性能仍未验收。

### 2026-09-24 Codex：候选中的消息输入与附件（尚未编译/发布）
- 独立工作目录 `C:\tmp\reader-native-runtime-20260924`，分支 `codex/native-pdf-runtime-20260924`；没有修改 Claude 检出或已安装程序。
- 本批未提交代码包含原生轮次/卡片事务、PDF 导航、设置、IME 回车，以及侧栏文件附件。附件沿用输入框上方的选区附件样式，按格式显示图标，图片有缩略图并复用 PencilKit 全屏标注；可仅发附件或附文字。普通后台接收真实图像输入，复习独立对话接收文件引用。
- Windows 新增受现有 Tailscale 身份校验保护的附件上传/预览/下载端点；原件保存到 `%LOCALAPPDATA%/BWReader/assistant-attachments`。附件与消息都使用稳定编号；发送结果不明时不自动再起一轮。
- 验证：附件/普通打字 8 项、既有轮次 52 项、原生轮次/清空边界 12 项通过。最新成功完整 Apple 编译仍是 `c4c2cebe` 的 run `35967857219`，不包含这些 WIP；依用户要求，代码修改完成后才统一构建。
- 新附件功能必须与 ReaderPC 更新配套安装。未部署服务器、未出安装包、未上传 TestFlight，手绘/输入法/照片选择尚未实机验证；剩余会话/生成物操作及复习兼容控制器仍待迁移。语音 WebRTC 方案只调查，未修改传输协议。

### 2026-09-24 Codex：原生复习与附件整批验证
- 同一独立候选继续接入原生复习显示/评分恢复/卡库更新/出处导航；图库导入期间预留附件，防止漏发，IME 确认候选时不发送。
- 验证：C# Release 编译通过、附件打字 Python 检查通过；Reader Node 整批检查后的两条过时结构断言已修正并聚焦复验通过，vendor 与 App 资源已重新生成。
- 接下来整批检查 Swift 与 App 编译；没有上传 TestFlight、切换服务或声称整个迁移已完成，剩余兼容控制器和实机交互仍需收尾。

### 2026-09-24 Codex：原生查询编译通过、生成物操作继续迁移
- 已验证：Apple `35997315139`（`82f3a329`）原生检查与完整 App 编译通过；ReaderPC 0.1.296 运行，Direct 0.1.440 安装文件及安装后自检通过，原安装进程中断未留最终回执，详见迁移记录。
- 本批接入学习卡及知识/天气/新闻卡的原生拖放和原件读取，保留卡组空位、编号及完整正文；复习显示答案/展开直达原生队列，出处导航按同一轮次核对。
- 验证：聚焦 conversation-delta/review-candidate-ui 检查通过，vendor 与离线 App 资源已生成；新 Swift 变更留待整批 Apple 验证，未签名、未出安装包或上传 TestFlight。
- 下一步继续清理媒体/会话操作、复习控制与账户 outbox、启动和 PC 上下文兼容依赖；EPUB 正文与局部视频 WebKit 保留。不能宣称只剩打包。

### 2026-09-24 Codex：媒体原件及选中操作继续原生迁移
- 原生解析媒体资源、视频编号及地图数据，图片/视频落页走已有原生事务；选中、同组互斥和过期由 Swift 处理，网页只观察已提交状态并投递原有通知。
- App 不再生成语义卡正文或启动隐藏图片/地图组件；保留原索引、删除状态、出向焦点和局部视频 WebKit。
- 验证：聚焦媒体/上下文/对话 Reader 检查通过，vendor 与离线 App 资源已生成；本批 Swift 与前两批一并等待整批 Apple 构建，未出安装包或上传 TestFlight。
- 继续处理对话调度、原始事件投递、复习控制/账户 outbox 及启动/PC 上下文依赖；没有扩展功能范围或改语音协议。

### 2026-09-24 Codex：账户待发队列原生接管（候选）
- 命令合并、批次捕获、成功确认与终态拒绝记录转到独立 Swift/SQLite transport 库；设备缓存回收不清除待发命令，旧副本不能重新激活已确认操作。
- 兼容入口同步返回原 mutationId，临时记录只有收到原生落库确认才删除；原生入口报错时保留记录，不切回网页发送。各操作仍走既有本地/服务器分流，不把本地操作误发到服务器。
- 验证：Reader Node 2483 项通过；新增 Swift 持久化、账户隔离和回执并发检查已列入下一次统一 Apple 构建，目前尚未运行；未出包、未上传 TestFlight。
- 继续迁移剩余复习/对话调度和生产命令的兼容入口；保留 EPUB 正文和局部视频 WebKit，不改语音协议。

### 2026-09-24 Codex：复习回答选择与草稿模式（候选）
- 原生按钮直接修改 Swift 选区状态，按当前复习轮次和卡片核对；回答段落合并、整条覆盖段落和过期清理由同一份原生状态处理。草稿准备先等选区提交，不使用过期选择。
- 草稿详细程度直接更新原生队列并使旧预览失效；迟到的旧显示回执不会清掉新预览。原有卡片身份、原文及评分流程保持。
- 验证：Reader Node 2484 项通过；Swift 选区/段落/过期行为检查留到最后统一 Apple 编译。目前最新 Apple 成功仍为 `82f3a329`，没有新 App 包或 TestFlight 上传。
- 未完成：对话事件/调度、复习其余控制与投递衔接、启动和 PC 上下文兼容路径。复习回答注册仍由现有对话适配器提供，不能称整个迁移完成。

### 2026-09-24 Codex：原生生成物显示数据与流式接收（候选）
- 工具结果、知识/天气/新闻及 Anki 字段改由 Swift 从原件整理；网页只传操作标识和原件，按 gid 找学习卡，避免同条消息多组卡串组。
- 普通 SSE 增量跳过书页提交适配器；只有 actions 等待保存回执，独立记录写入序号并保留原事件顺序，不把不完整回执当成功或重放。
- 验证：对话/流式聚焦检查通过；Reader Node 整批仅旧 POST 重试定时断言失败，单独复跑通过。新增 Swift 检查留待最终统一 Apple 构建，未出包或上传 TestFlight。
- 仍需迁移对话排序/操作登记、文档修改适配器、复习调度及启动/PC 上下文；本批不涉及 Windows 服务切换。

### 2026-09-25 Codex：PDF 对话准备/写入及复习状态交接（候选）
- PDF 请求直接从原生快照和字符数据准备；卡片编号与原生显示共享排序。流式 actions 直接调用原生事务/卡片 saga，网页只观察回执并保留文件操作租约。
- 复习切卡、评分暂存、撤销和显示答案由 Swift 操作当前队列；保留先提交上一评分、失败不推进及暂存可撤销的原有行为。
- 验证：聚焦与全量 Reader Node 通过；vendor 和离线 App 资源已更新。Swift 新检查留到最终统一 Apple 构建，未出包或上传 TestFlight。
- 待续：对话操作/事件登记、复习效果与模式调度、启动导入和 PC 上下文；未改服务器运行版本，无 BWAB_CLI，不使用旧协作邮箱。

### 2026-09-25 Codex：复习模式/取卡与改进草稿原生调度（候选）
- 模式、范围、重载及草稿按钮直达 Swift；PDF 上下文和草稿卡面/所选回答从原生读取。兼容层只等待旧评分效果、观察结果，不再因模式通知重复取卡。
- 保留原缓存身份、失败评分恢复、显式预览确认及未知写入不重投；未保存评分会阻止换队列。迁移文档已列出固定收尾清单，避免把残留依赖反复当成新任务。
- 验证：聚焦及全量 Reader Node 通过；vendor/离线资源已重建。新增 Swift 检查留待最终统一 Apple 构建，未出包、未上传 TestFlight、未重启服务。
- 下一步继续评分效果/删除、对话调度、账户投递及启动/上下文；EPUB 正文和局部视频保留 WebKit。无 BWAB_CLI，不回退旧协作邮箱。

### 2026-09-25 Codex：复习评分与删除效果原生接管（候选）
- Swift 接管暂存评分提交、删除、结果恢复及队列更新；本地评分和待投递命令同事务保存，账户投递失败保留原编号，未知外部写入不盲重发。网页只观察结果和兼容通知。
- 验证：Reader Node 2498 项通过，vendor/离线 App 资源已重建；新增 Swift 事务、去重及恢复检查留到最后统一 Apple 构建，本批未编译、未出包、未部署。
- 继续固定清单中的对话调度、账户发送及启动/阅读状态；不新增功能，不改语音配置，EPUB 正文及局部视频仍保留 WebKit。

### 2026-09-25 Codex：对话原件引用与卡片上下文（候选）
- 结构化轮次及普通流式回复引用 Swift 原件；媒体移除/选中、整张 Anki 与语义卡带入及附件取消由原生状态处理，保留原索引、父子去重及失败通知。
- Reader Node 2500 项通过，vendor/离线资源已更新；新增 Swift 案例待最终统一 Apple 编译。没有新包、部署、TestFlight 上传或生产数据改动。
- 消息成员/排序及部分操作登记仍需迁移；继续固定清单，不把本批当作整个对话迁移完成。无 BWAB_CLI，不回退旧协作渠道。

### 2026-09-25 Codex：账户待发命令的原生发送（候选）
- Swift 接管本地/远端分流、逐项回执和重试；原编号及账户隔离保留，本地写入与复制命令同事务，EPUB 设备续读索引保留，其他书籍命令延后。
- Reader Node 整批通过，vendor/离线资源已重建；新增 Swift 事务、重试和迟到回执案例等待最后统一 Apple 编译。未出包、上传或部署。
- 下一步继续对话排序/操作登记及启动/阅读状态固定清单；仅改独立候选目录，无 BWAB_CLI，不回退旧协作渠道。

### 2026-09-25 Codex：侧栏评分与卡片操作标识（候选）
- Swift 接管卡片控件标识和独立 Anki 评分，原 aid、事务回滚、未知结果保护保留；网页只观察结果及发出去重通知。
- 聚焦 Reader Node 检查通过；新增 Swift 检查留待最后统一 Apple 编译。未发布、未部署、没有新安装包。
- 继续对话成员/顺序及启动/阅读状态收尾，不扩展功能范围。

### 2026-09-25 Codex：对话生命周期与顺序（候选）
- Swift 按生产者事件维护消息成员和顺序；保留晚到转写、历史原子替换、流式轮次和取消，不再从隐藏 thread 子节点扫描成员。登记入口提前到 documentStart。
- Reader Node 整批通过；历史取消/同编号替换及范围隔离另有原生案例，待最终 Apple 编译。未出包、未上传或部署。
- 继续剩余对话动作、启动与 ReaderPC 状态；旧资源句柄尚有兼容入口，不能据此宣布全量迁移完成。

### 2026-09-25 Codex：原生迁移候选 1.1.85（974）已上传 TestFlight
- 改动：独立分支 `codex/native-pdf-runtime-20260924` 的 `eb3747d4` 已签名出包。对话、复习、账户发送、启动/阅读状态及 PDF 工具栏收尾已接原生服务；EPUB 正文及局部视频保留 WebKit，兼容生产者协议仍保留。
- 验证：Actions `36051387988` 的全部原生检查、完整模拟器编译、设备归档校验、IPA 导出和 TestFlight 上传成功。四项测试样本/整数类型问题已修正，没有放宽生产校验。构建产物 `10831562157` 含 IPA/日志。05:17 JST 查询 `36053870164`：Apple 上传状态 `PROCESSING`，内部测试组 `me` 接收所有构建，尚未确认可安装。
- 未做：没有运行实机滚动/耗电/Pencil/多设备验证，没有更换弱网语音协议，没有部署 Windows 或独立扩展。交接总脚本仍有 Windows 白名单、网页网络审计标记及扩展/PWA 清单旧门禁失败，已记录到迁移文档；不把 iOS 成功扩大成所有管线通过。
- 下一步：Apple 处理完成后安装 1.1.85（974），优先验收 PDF 连续翻页、词组保存/掌握、注解/Anki 显示与侧栏拖放、附件手绘及中文输入。代码收尾与出包已完成，不再以同一固定清单继续拆新任务。

### 2026-09-25 05:33 JST：974 已可供内部 TestFlight 安装
- App Store Connect 只读查询 `36055643813`：1.1.85（974）`processing_state=VALID`、`internal_build_state=IN_BETA_TESTING`、上传 `COMPLETE`，关联内部组 `me`。
- 没有重新构建、上传或修改测试组；外部状态仍为 `READY_FOR_BETA_SUBMISSION`。实机体验待用户安装验收。
