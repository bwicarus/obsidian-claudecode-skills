...
### 2026-08-23 跨页锚定与 Direct 离线可靠投递候选（0.2.137 / Direct 0.1.178 / App 1.1.82）
- **修复**：AI 可按同书离屏页原始字符坐标直接锚定；Direct 在 App 后台、非当前页或来源短暂离线时持久排队，恢复后幂等重放，手动自由卡绑定复用稳定 ID 且只对网络/5xx 重试一次。
- **一致性**：结构化快照同时保留 Markdown 阅读流与 `pageChars` 原始坐标；锚定卡确定性 `noteId/mutationId`，初始权威列表水合失败时关闭写入，恢复与并发加载均不重复建卡。
- **验证**：共享生成物、Reader Node `1704/1704`、Direct Release 编译/完整自检、网络审计（0 new debt，收紧 3 项旧基线）已通过；Windows 全量仅命中已登记 POSIX/fixture 环境基线。
- **边界/下一步**：未改 ReaderPC 与数据 schema；提交推送后执行 Pi Linux 原子门禁/部署、公开 WebExt、Direct 原子安装及 TestFlight 上传，并回填事务与健康结果。
