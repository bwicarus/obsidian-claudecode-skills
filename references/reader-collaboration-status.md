...
## Codex：App 本机 Realtime 启动修复候选 0.2.84 / 1.1.22（2026-08-10 JST）
...
- **怎么验的**：相关合同 26 项及 Reader Node 全量通过；Windows Python 仅命中既有 `fcntl` 平台基线，Pi/Linux 完整门禁与 macOS 编译归档待本候选提交后执行。
- **下一步**：发布 Pi/扩展与 TestFlight 后，在设置替换同一 key 看到“Keychain 与 OpenAI 均已验证”，再点普通电话；若仍失败，界面会直接给出准确阶段与错误。
