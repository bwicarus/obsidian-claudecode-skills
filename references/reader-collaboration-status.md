...
- **改了什么**：PDF/EPUB 不再仅凭原生能力标志禁用网页画笔；PencilKit 未就绪或 hit-test 放行时，网页层继续绘制，避免双端同时拒绝输入。
- **怎么验**：专项合同与 JS 语法通过；Reader 正式部署后由统一 App `1.0.88 (1)` 同步当前颜色、粗细与橡皮工具。
- **边界/下一步**：Windows `0.1.95` 语音链路未动；实机验证多颜色、两种粗细与橡皮。

## Codex：电脑客户端设置显示并控制 Codex 语音（2026-08-06 JST）
- **改了什么**：设置页读取 Windows 提供的 Codex 麦克风/语音状态，并用独立的目标状态按钮开启或关闭；刷新只读，不发送 START/STOP，合成点击不能触发快捷键。
- **怎么验**：共享运行时与统一 App 分支逐字一致；电脑语音专项合同和 Reader Node 主体通过，全量仍仅有既存 popup 测试夹具四项失败。
- **没做/下一步**：未带入网页视口、扩展 call/content 或 App 版本改动；当前仅本地提交候选，待上游合入后由 Pi 原子预检与部署。
