...
- **所有数学内容必须用 LaTeX 包裹，禁止任何裸写**。这是硬性要求，不是可选项。
  「数学内容」包括但不限于：带下标/上标的变量（`b_1`、`e^x`、`p_i`、`V_n`）、
  函数与算子名（`span`、`dim`、`det`、`ker`）、希腊字母、省略号（`\dots`/`\cdots`）、
  分式根号、集合/区间记号（`\{\,\}`、`(a,b]`）、求和积分、任何含 `_` `^` `/` `∑` `∈` 的式子。
  哪怕只是单个带下标的字母，也必须包裹。
- LaTeX 分隔符**必须**使用 Anki 默认 MathJax 支持的形式：
  - 行内公式：`\(...\)`，例如 `\(V_n\)`、`\(\frac{d}{dx}f(x)\)`、`\(e^x\)`
  - 块级公式：`\[...\]`，例如 `\[\sum_{i=1}^n x_i\]`
  - **禁止**使用 `$...$` 或 `$$...$$`，这两种分隔符在 AnkiMobile / AnkiDroid 默认配置下不会渲染，公式会以原始文本形式显示。
  - cloze 卡的挖空可包含 LaTeX：`\(\frac{a}{b} = {{c1::\frac{c}{d}}}\)`。
- 正确 / 错误对照（务必照做）：
  - 错 `span(b_1,...,b_N)` → 对 `\(\operatorname{span}(b_1,\dots,b_N)\)`
  - 错 `e^x 在 0 处的泰勒级数` → 对 `\(e^x\)` 在 \(0\) 处的泰勒级数
  - 错 `{p_1b_1+...+p_Nb_N: p_1,...,p_N in F}` → 对 `\(\{\,p_1 b_1+\dots+p_N b_N : p_1,\dots,p_N\in F\,\}\)`
  - 错 `dA/dx` → 对 `\(\dfrac{dA}{dx}\)`
- 必要时在公式后用中文简述符号含义。
...
不需要制卡时（`sections_processed` 仍须填写）：

```json
{
  "should_create_cards": false,
  "reason": "主要是学习安排，不包含需要长期记忆的知识点",
  "sections_processed": ["节名1"],
  "cards": []
}
```

`sections_processed` 必须列出本次处理的**所有**节名（即使某节不制卡）。脚本凭此字段更新节哈希，未列出的节下次仍会被视为待处理。
