...
| Claude CLI | `/root/.local/bin/claude`（native installer;2026-06-10 卸掉了挡道的 npm 版 /usr/bin/claude,PATH 加了 ~/.local/bin,另有 `/usr/local/bin/claude` symlink 兜底——旧 shell/非登录 shell 也找得到,自更新只改 ~/.local 内层链接不受影响。⚠ VPS 的 root 走 **nvm**,nvm bin 在 PATH 最前——在 nvm 环境下 `npm install -g claude-code` 会再造一份挡住原生版(当天就发生了,已删);修完后老 shell 还要 `hash -r` 清 bash 路径缓存） | `/home/bwicarus/.local/bin/claude`（native installer，经 PATH 调用，非 /usr/bin） |
