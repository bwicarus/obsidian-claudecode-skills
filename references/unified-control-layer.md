...
### 自动消失 & 标记呼吸(138,用户实测两个 bug)

- **倒计时到点 = 卡片完全消失**(不是收起成小方块)。**唯一豁免 = 长按选中**(紫边)。
  碰它(点/拖/切形态)= 你在看它 → 倒计时**重新开始**;松开不管它,照样会走。
  统一入口 `_armAuto(c, el)`(rc-voicecall);`rc-toolchip` 的 `AUTO_COLLAPSE` 已退役。
  ⚠ 旧 bug:`pointerdown` 里写的是 `clearTimeout(c.t); c.t = null`(「碰了=在读:取消自动消失」)——
  **点一下倒计时就被永久掐掉**,卡片再也不会消失。
- ⚠ **标记闪烁**:`_cardPush` 曾把 `busy`(正在干活的呼吸动画)**写死**在标记的 class 上。
  工具卡有状态机(`paintSum`)会摘掉它,**结果卡(天气/图/视频)根本没有状态机** → `busy` 永远挂着
  → 标记一直在 0.45↔1 之间呼吸 = 用户看到的"点成小方块后一直闪"。
  修:`busy` 改由 `opts.busy` 显式给(工具卡出生即 busy,结果卡不给)。
...
### 工具调用详情窗(139,用户设计)—— 旧「!」面板的接班人

- **长按**工具卡的**长条**、或流程图里的**任一小长条**(`.vc-fn`)→ 打开详情窗(`#vc-dtl`)。**单击展开的功能保留**。
- 格式复用旧「!」面板:**每条流程一行** `[名称] [模型 chip] [耗时]`,**点名字展开该条的细节**(markdown)。
  用了 AI 模型的那步,除耗时外还显示**模型选择**(`chip.model` / `step.model`,后端 `_tool2(model=_gm, action=_ga)` 透出)。
- **载荷写人话,不倒 JSON**:`argLines()` 把参数渲染成「**文字**:东京 天气」这样的「格式 + 内容」行
  (`ARG_LABEL` 映射 query→文字 / page→页码 / deck→牌组 …)。
