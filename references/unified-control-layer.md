...
`AI 请求 → 各处理步骤 → 结果` 每一环是一个**可点开的小方块**(`.vc-fn`),之间用**带箭头的连接线**(`.vc-fw`)表示数据传递。点方块展开它经手的载荷(`.vc-fp`):

- **AI 请求**节点 → 参数 JSON(pretty-print)
- **步骤**节点 → 该步的详情
- **结果**节点(默认展开)→ markdown / 公式 / 图正常渲染(走 `RC.assistant.renderMd` + MathJax;侧栏未挂载时用 `miniMd` 兜底,**绝不吐原始文本**);制卡则是完整卡片预览(正反面翻页 / Cloze)

- **标记本身 = 形态控制按钮**(`.vc-card-dot`,坐落在卡片**左上角**),单击循环三态;它同时是收起态下的**拖动把手**(那时 `.vc-card-hd` 隐藏,没有它就拖不动)。`.vc-hasdot` 让标题给标记让位。
- **左上角锚定**:工具卡走独立图层 `#vc-tlayer`(全屏,`left/top` 定位 + `transform-origin:0 0`),不进右下角那个 `#vc-cards` 堆叠层——否则展开时卡片往左上长、标记会乱跑。三态切换 = 以标记为原点的拉长/展开动画。落点由 `_tSpot()` 按画面当前占用打分(交错重叠)。
- **执行类(不产出内容)**:完成 → **浮层卡即消失**;**只有出错才留在页面上**(红色),点标记展开看错误详情。侧栏那条记录保留。
