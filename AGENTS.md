...
打印一行 JSON，`path` 就是刚拍的照片，直接读它。约 5 秒
（连拍 8 帧挑最清晰的一张，`sharpness` / `brightness` 一并返回）。

现有的：`pi`（C920，在 Pi 上）/ `usb`（书桌）/ `builtin`（机身后置 13MP）
/ `usb5m`（机身前置）。id 就是 `snap` 的参数。

### ⚠ 什么时候该用

**只在"看一眼就能定"的时候用，而且是你自己决定要看的时候。**

- ✅ 用户问「桌上那个是什么」「灯关了没」「垃圾拿出去了吗」——
  这类问题**只有画面能答**，文字上下文里根本没有
- ✅ 你要做的判断依赖现场状态，而且**拍一张就够**
- ❌ **不要为了"顺便了解一下"而拍。** 图像不进快照载荷，正是为了不让你
  每取一次上下文就被塞一张家里的照片 —— 这条是用户明说的边界
- ❌ 不要连拍、不要轮询。要看的是"变化"的话，那是另做一套的事
...
这是**家里的**摄像头。拍到的东西只用于回答当下那个问题：不要转述与问题
无关的画面内容，也不要把图或它的描述写进会长期留存的地方（笔记、待办
正文、日志），除非用户明确要求。
