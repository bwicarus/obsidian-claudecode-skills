...
家里有四台可用（`pi` / `usb` / `builtin` / `usb5m`），你可以自己拍一张来看：

```bash
python %LOCALAPPDATA%\BWReader\camera_capture.py snap usb
```

⚠ **用法和边界见 skill `look-through-camera`**（什么时候该拍、什么时候
不该拍、拍不到怎么办、隐私）。这里只留一行指路 —— 细节放 skill 里按需
加载，不必每一轮都占着上下文；但**必须留这一行**，否则它根本不知道
有摄像头这回事（2026-08-27 做好、到 08-29 都没人告诉它，就是这么来的）。
