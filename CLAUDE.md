...
- `references/sticky-notes-design.md` — **便签系统**（PDF/EPUB 共享 `rc-stickynote.js`）：共享层只管策略/状态机，锚定/挂载 per-reader 经 adapter hook，**锚在内容坐标系**（严禁 fixed+JS 跟滚）；后端 `/pdf/api/notes` + `/api/note-composite`，`state/reader-notes/`
