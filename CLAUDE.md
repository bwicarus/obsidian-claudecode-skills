...
- `references/activity-ledger-design.md` — **活动账本设计稿**（2026-08-22，⚠ 未动工，§6 待用户拍板）：注意力**转移记录**（时间/界面/内容id/焦点）+ 生成物生命周期 + 半自动压缩。⚠ **动工前必读 §2 现成资产盘点** —— 这个想法一大半的地基已经在跑（`attention_profile.py` 的 events.db + append-only 账本、两套 id 发号器、`learning_situations.py` 的「门槛→聚合→AI 只一次→证据水位防重复」、`read_material('anki:<cid>')` 的 id→实体内容），不要重造。真正空白的只有四件：生成物 TTL（全仓 6 处过期删除**没有一处**按「是否被固定保存」判定）、修改/删除的版本记录、`surface` 界面维度（App/桌面/扩展今天在任何记录里都还原不出来，**服务端补不出来、必须客户端上报**）、地点（零基础设施）。⚠ 用户要的「删原记录插总结」与三条写死的铁律（账本永不删 / 写时不过滤 / 只转状态不删）方向相反 —— 建议冷归档而非删除，理由见 §6。⚠ 注意跟 `attention-kb-design.md` 区分：那份是注意力**画像**（我最近关注哪些概念），这份是活动**账本**（我何时在哪看了什么）。
