...
## Anki 制卡流程
Anki 制卡由 agent 与脚本分工完成：agent 负责阅读、判断和生成卡片 JSON；脚本负责 records 判断、AnkiConnect 同步和写入记录。

### 批量列出待处理笔记
```
C:\Users\bwica\AppData\Local\Programs\Python\Python313\python.exe scripts/anki_from_note.py --list --dir <目录路径> --recursive
```

### 读取单篇笔记制卡上下文
```
C:\Users\bwica\AppData\Local\Programs\Python\Python313\python.exe scripts/anki_from_note.py --read <笔记路径>
```

agent 根据输出中的 `references/anki-selection-rules.md`、`references/anki-card-format.md`、`references/vault-structure.md` 和当前笔记内容判断是否制卡，并生成 JSON。

### 同步卡片到 Anki
```
C:\Users\bwica\AppData\Local\Programs\Python\Python313\python.exe scripts/anki_from_note.py --sync <笔记路径> --cards-json <cards.json>
```

如果不需要制卡，写入 no_cards 记录：
```
C:\Users\bwica\AppData\Local\Programs\Python\Python313\python.exe scripts/anki_from_note.py --sync <笔记路径> --no-cards-reason "主要是学习计划，不包含需要长期记忆的知识点"
```
