...
- `references/epub-on-epubjs-migration.md` — EPUB 阅读器迁到 **epub.js** 成熟地基（2026-06-27 起，`epub2*.js`）：手搓 reflow（`epub-html.js`）的布局/tap 坑太多，改用 epub.js 但**功能一个不少、从 PDF 阅读器最原始 `reader.src/*.js` 搬**（不搬已 drift 的手搓代码）
...
| **EPUB 阅读器** | `https://bwicarus.taile44d0c.ts.net/pdf/epub/view` | 网页 EPUB 阅读器（后端同在 `pdf_reader.py`，`/api/epub-*` 路由）：**epub.js** 成熟地基（`epub2*.js`）+ 从 PDF 阅读器 `reader.src/*.js` 搬齐全部功能（高亮/查词/AI 侧栏助手/振假名/整章翻译/图描述/语法），走 `window.RC` 统一控制层。完整迁移见 [`references/epub-on-epubjs-migration.md`](references/epub-on-epubjs-migration.md) |
| **阅读器共享功能**（PDF+EPUB） | 同上两个阅读器 | 经 `window.RC` 统一控制层（`references/unified-control-layer.md`）共享：**插入页/用户页**（PDF 真插入 PDF 文件 + overlay 即时编辑；EPUB `.ep-usec` 空白段所见即所得，`/api/userpages`+`/api/pdf-insert-page`）、**收藏夹**（`/api/favorites`→`state/reader-favorites.json`；统一化 v5 物化成真 EPUB `/pdf/fav/view` 全功能打开 + AI 认收藏集 + NotebookLM 入口）、**便签**（`rc-stickynote.js`，`/api/notes`）、**手写墨迹**（`/api/epub-ink`）、**阅读位置服务端化**（`/api/reading-pos`→`state/reader-positions.json`）。详见 [`references/reader-userpages-favorites.md`](references/reader-userpages-favorites.md) / [`references/sticky-notes-design.md`](references/sticky-notes-design.md) |
...
- ⚠ **前端 `_server_deploy/static/pdf/*`**（`reader.js`+`reader.src/*.js` 源、`rc-*.js` 共享控制层、`pdf-adapter.js`/`epub-html.js`/`epub2*.js` 各 adapter、`rc-userpages.js`/`rc-favorites.js`/`rc-stickynote.js` 等）**只由 nginx(443) 从 `/var/www/html/static/pdf/` 服务**（`immutable` 缓存）；`127.0.0.1:5000` 的 Flask `/static/` 是**陈旧副本**——改前端必须部署到 nginx 静态目录，直连 :5000 看到的是旧的
