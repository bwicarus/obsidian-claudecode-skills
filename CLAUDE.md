...
2026-07-25 那版边界（普通网页装扩展才启用完整 BW 功能；PWA 只读 PDF/EPUB/导入 HTML/Markdown/收藏书）**已被本文件末尾「🎯 产品边界（2026-08-18）」收窄：只做 App 与扩展，PWA 与网页端不再投入**。本节只用来查能力归属、运行时冲突和视觉冲突登记，不要拿它当现行产品边界。
那版边界覆盖的书籍类型（PDF、EPUB、导入 HTML/Markdown、收藏书）仍是今天的范围；但其中「无扩展时由 PWA 提供完整界面」已作废（PWA 2026-08-14 废除，页面路由 410），这些书现在由 App 承载，浏览器里由扩展接管
...
- `references/ambient-jev.md` — **iPad 环境旁听 + 独立 jev 判断 + 嘈杂环境人声隔离 + 熟人声纹**（2026-09-26）：本机转写 + FluidAudio 分人 → `/api/ambient/judge`（`_server_deploy/jev_judge.py` 独立模块，任何时候可用，与语音无关）→ 危险录音 / 存原声 / AI 解答 / 滚动摘要；通话中改接通话上行；通话多人时只放行「我」
...
- `references/systemd/*.service|*.timer` — 服务器 systemd unit 文件副本（xvfb-99 / anki-headless / obsidian-sync / qa-server / webapp / bwicarus-daily.service|timer / anki-sync-refresh.service|timer / bwicarus-quick-sync.service|timer / book-ocr.service / book-ocr-watchdog.service|timer（timer 加了 `OnActiveSec=15min`：只有 OnBootSec+OnUnitActiveSec 的 monotonic timer，开机很久后才 restart 会 Trigger:n/a 永不触发，`Persistent=` 只救 OnCalendar） / **bwicarus-backup.service|timer**（每日 03:30 跑 `scripts/backup_data.sh`：webapp/data 账号数据 + claude/state 学习状态 → `~/backups`，保留 14 份） / **push-big-files.service|timer**（仅 Pi：每 4h 把 vault 里 >200MB 文件推到 PC，OnBootSec=5min + OnUnitActiveSec=4h + Persistent，User=bwicarus） / **yolo-figures.service|timer**（每 6h 闲时 DocLayout-YOLO 框图，跑 `doclayout-venv/bin/python yolo_figures.py --all`，Nice=19/CPUWeight=20 低优先级，Pi CPU ~6.7s/页，只跑开了「插图描述」的书、幂等；填 `figures_geom` 供图徽标/裁图描述 gate） / **figures-describe.service|timer**（每晚 02:30 off-peak 跑 `scripts/describe_figures_batch.py --all`：读 YOLO 框做 gate=0跳过/1裁图/≥2整页 → 夜间用 off-peak 额度生成插图描述，Nice=15/CPUWeight=30，`Persistent=true` 关机补跑；「YOLO 先 gate→夜间裁图描述」流水线反转的**消费端**，前置由 yolo-figures.timer 每 6h 填好 `figures_geom`））
...
- `references/unified-control-layer.md` — **统一控制层 `window.RC`**：PDF / EPUB 阅读器**共用一份控制层代码**（`rc-*.js` 共享层管策略/状态机 + 每个阅读器一个 adapter：`pdf-adapter.js` / `epub-html.js`）。设计铁律（用户拍板，凌驾一切）=让中间层去适应旧代码、**完全复用 PDF 阅读器原有全部能力**，不为不同阅读器另造上层建筑；高亮/下划线/选区/内容输入输出统一接口
- `references/epub-on-epubjs-migration.md` — ⚠ **历史归档，勿据其推断现状**：2026-06-27 曾把 EPUB 阅读器迁到 epub.js（`epub2*.js`），2026-07-06 该逃生分支整线删除；现役唯一实现是手搓主文档版 `epub-html.js`（功能超集，从 PDF 阅读器最原始 `reader.src/*.js` 搬齐，不搬已 drift 的手搓代码）。只在追溯当时的取舍时读它。
- `references/reader-userpages-favorites.md` — 阅读器**插入页（用户页）+ 收藏夹**完整设计与实现：PDF 插入页=**真插入 PDF 文件** + overlay 即时编辑（存边车 `state/reader-userpages/<sha>.json` 不碰 PDF、始终覆盖层显示不降级、乐观新建即全功能页：手写/查词/精确高亮全可用、停编/关书后台异步写回 PDF）；EPUB 插入页镜像 PDF（`.ep-usec` 整页空白段 + 所见即所得 + 编辑态拖下边缘调高存 `h` 字段）；**收藏夹统一化 v5**=物化成一本真 EPUB（`state/reader-fav-epub/<fid>.epub`）用**完整 EPUB 阅读器**全功能打开 + 服务器自动后台 build/脏重建/秒开 + 插入页可收藏（`kind:'userpage'`）+ PDF 条目 char-layer 式自定义选择 + AI 认收藏集（`_fav_fid`/目录概览/相邻性/`read_source_page` 工具，`state/reader-fav-meta/`）+ NotebookLM 一键入口（总结/串联/共同点）
- `references/sticky-notes-design.md` — **便签系统**（PDF/EPUB 共享 `rc-stickynote.js`）：共享层只管策略/状态机，锚定/挂载 per-reader 经 adapter hook，**锚在内容坐标系**（严禁 fixed+JS 跟滚）；后端 `/pdf/api/notes` + `/api/note-composite`，`state/reader-notes/`；**卡片便签在 PDF 上是「词锚」形态**（2026-08-20 用户拍板，取代圆球）：绑定的词描边 + 右上角页内序号，点词展开真卡；词锚存 `note.card.bind` **不存 anchor**（anchor 是逐字段重建，加字段会被静默 strip）；五个必须一起做的时机见该文档末节，少一个都是「看着像正常」的错位
...
- `references/codex-notification-channel.md` — **Codex 主动通知通道**（2026-09-10 实测定稿）：通道=可枚举并自证的命名管道 + 目标线程，**不需要 Codex 配合/钩子/环境变量**就能建。⚠ 动手前先读那六个坑 ——管道名每次重启就变、ConnectAsync 不抛 FileNotFound 而是等到超时、并存管道里只有一条是活的、list_threads 信封要真实 threadId、updatedAt 混着秒与毫秒、`CODEX_APP_TOOLS_PIPE_PATH` 只在桌面应用自己的会话进程里。另记：**推送不能用来起通话**（结构性闭环），起通话由桥自己按
...
- `references/ios-reminder-delivery.md` — **iOS 提醒送达架构**（2026-08-26）。⚠ **动"提醒/通知/闹钟"之前先读**：已建成三条并行通道（到点本地通知 / 苹果提醒事项+EKAlarm / AlarmKit 系统闹钟），失效场景不重叠；排程发生在 App 内**和 Widget 内**两处（widget 那条补的是"用户不开 App 就没人排"的洞）。含 AlarmKit 三条反直觉事实（**没有 entitlement**、widget extension 只在 countdown 呈现时才必需、stopIntent 可选）、CI 用 `macos-latest` 无 pin 导致 `canImport` 可能**静默**把整块编译掉的隐患、以及被调研推翻的两个设想（APNs 该发 **alert 推送而非静默推送**；发送方该是 **Pi 不是会睡的 Windows 笔记本**）。附诊断出口速查表：没响时先看投影回执的三段状态与桥的 `reader-output-pickup.log` 里的 `widget-fetch` 行
- `references/watch-companion.md` — **手表伴侣 app**（watchOS，2026-08-27）：卡片/待办/语音桥开关。一句话架构=**手表看到的是手机的镜子**（手表够不到 tailnet，iPhone 是唯一数据源，走 WatchConnectivity）。⚠ 含三个会烧掉构建的坑：XcodeGen 2.46.0 把手表 app 嵌进 `Watch/` 而 Xcode 16+ 只认 `PlugIns/`（CI 有补丁 + 位置断言，**CI 绿灯不等于成功**）、项目级 `SUPPORTED_PLATFORMS` 会漏进手表 target、校验函数写死了 iOS 的最低版本与设备家族。**手表直接语音对话明确不做**（要开公网，不划算）
...
| **systemd 服务** | `/etc/systemd/system/` | `xvfb-99` + `anki-headless` + `obsidian-sync` + `qa-server` + `bwicarus-daily.timer` (01:00;原 04:00,73d8eb6 起提前错开时段) + `tailscaled` + `webapp` + `anki-sync-refresh.timer` (15min 拉手机复习数据) + `bwicarus-quick-sync.timer` (15min vault 状态同步) + `book-ocr.service` + `book-ocr-watchdog.timer` (日文 PDF OCR 后台 + 自检) + `bwicarus-backup.timer` (每日 03:30 `backup_data.sh` 备份 webapp/data + claude/state → `~/backups`，保留 14 份) + `yolo-figures.timer` (每 6h 闲时 DocLayout-YOLO 框图填 `figures_geom`) + `figures-describe.timer` (每晚 02:30 off-peak `describe_figures_batch.py` 按 YOLO 框 gate 生成插图描述) + `artifact-lifecycle.timer` (每日 04:10 生成物冷归档判定 `artifact_lifecycle.py --from-config`,开关在 server-config `artifact_lifecycle` 段,默认 dry-run 只报告) + `mcp-server.service` (MCP 门面 `--http 8766`(⚠ 8765 是 AnkiConnect 默认端口,撞过车;权威见 `references/systemd/mcp-server.service`),外部 agent 经 nginx `/mcp` 反代控制整个 App;详见 `references/mcp-server.md`) + `watch-voice.service` (手表语音桥的 Pi 侧减震器 `:8768`,手表经 Funnel 接进来、Pi 自己按 50Hz 恒定节拍推给 Windows 语音桥;⚠ 它**不是转发器**,序号/心跳都由 Pi 自己生成,手表来的只有 start/stop/ping 三个枚举 —— 为什么这么设计见 `references/watch-voice-bridge.md`)。**仅 Pi** 另有 `push-big-files.timer` (每 4h 把 vault 里 >200MB 文件推到 PC，绕过 Obsidian Sync Plus 单文件 200MB 上限) |
...
| **PDF 阅读器** | ⏸ 网页入口 `/pdf/` 已于 2026-08-14 退役（410，见 `_server_deploy/reader_pwa_retirement.py`）；能力现由 App + 扩展承载，Pi 只剩 `/pdf/api/*`。`https://bwicarus.space` 还是暂停中的 VPS，别拿它验证 | 阅读器能力清单（历史上是网页 PDF 阅读器）：PDF.js v4 + PyMuPDF char-bbox 选中（绕开 textLayer 偏移）+ AI 翻译/解释/问 AI（SSE 流式 + Markdown + MathJax）+ ECDICT 离线字典（单词秒查不耗 AI）+ **高亮编辑**（sidecar JSON、4 色板互斥激活态、点 cur 色取消颜色保留备注、popover 备注小框单行省略点击展开、iOS Mail 左滑右侧露出 🗑 删除）+ 草稿系统（AI 回答 + 选段 → 笔记/Anki）+ 设置面板（model/effort/debug/颜色管理）+ 右侧抽屉知识点关联 + **操作性 7 件套 F1-F7**（页码滑块 scrubber / 单页横滑翻页 / 整页翻译「译页」/ 全文搜索「🔍」/ 振假名+英文音标「あ」ruby 叠加 / 短多选→词组收藏作分词依据「📘词组」/ 日语生词下划线高亮）。完整文档 + 踩坑（含 §15 F1-F7）见 [`references/pdf-reader.md`](references/pdf-reader.md);**§16=2026-07 架构批次(静态抽取/SSE总线/续读仲裁/顶栏统一)必读** |
| **EPUB 阅读器** | ⏸ 网页入口 `/pdf/epub/view` 已于 2026-08-14 退役（410，见 `_server_deploy/reader_pwa_retirement.py`）；能力现由 App + 扩展承载 | EPUB 阅读器能力清单（后端同在 `pdf_reader.py`，`/api/epub-*` 路由）：手搓主文档版 `epub-html.js`（2026-07-06 起唯一实现；epub.js 逃生分支 `?engine=epubjs` + `epub2*.js` 套件已整线删除）+ 从 PDF 阅读器 `reader.src/*.js` 搬齐全部功能（高亮/查词/AI 侧栏助手/振假名/整章翻译/图描述/语法），走 `window.RC` 统一控制层。完整迁移见 [`references/epub-on-epubjs-migration.md`](references/epub-on-epubjs-migration.md) |
| **阅读器共享功能**（PDF+EPUB） | 同上两个阅读器 | 经 `window.RC` 统一控制层（`references/unified-control-layer.md`）共享：**插入页/用户页**（PDF 真插入 PDF 文件 + overlay 即时编辑；EPUB `.ep-usec` 空白段所见即所得，`/pdf/api/userpages`+`/pdf/api/pdf-insert-page`）、**收藏夹**（`/pdf/api/favorites`，owner=pi；统一化 v5 物化成真 EPUB `/pdf/fav/view` 全功能打开 + AI 认收藏集 + NotebookLM 入口）、**便签**（`rc-stickynote.js`，`/pdf/api/notes`）、**手写墨迹**（`/pdf/api/epub-ink`）、**阅读位置**（`/pdf/api/reading-pos`）。⚠ **除收藏夹外这一行的路由在 App 内全部本地执行**（owner=local + `native-local-runtime.js` 有本地分支）：权威数据在 App 本地，`state/reader-*.json` 只是 Pi 中继时的副本。改这些能力要改 `native-local-runtime.js` + 出 TestFlight 构建；改 `_server_deploy/*.py` 只影响桌面/扩展表面。动手前先跑 `python3 scripts/where_does_this_route_run.py <路由>`。详见 [`references/reader-userpages-favorites.md`](references/reader-userpages-favorites.md) / [`references/sticky-notes-design.md`](references/sticky-notes-design.md) |
...
**控制面板源码**（全部在 git。`app.py` / `control.py` / `static/nav.js` **在部署清单内 → 走 `scripts/deploy_reader.sh`**；只有 `templates/control.html` 在清单外，才手工 cp + restart）：
...
**阅读器 / AI 助手 / 语音源码**（都在 git。⚠ 这些文件**全在部署清单内 → 必须走 `scripts/deploy_reader.sh`**，不要手工 cp + restart；`qa_server.py` 在清单外，才手工 cp。部署前先用 `scripts/where_does_this_route_run.py` 确认要改的路由不是 App 内本地执行）：：
...
- `_server_deploy/qa_server.py` → 服务器侧 iPad 截图问答 daemon + cmd_server 入口（复用 `_client/core/qa_browser.py` + `cmd_server_thread.py`，即 `qa-server.service`）
...
- `_server_deploy/fitness.py` → Flask blueprint(`/private/fitness/*` 页面 + `/api/fitness/*` API,32 个 API endpoint + 5 页面路由)
...
- ⚠ **Pi 是什么、不是什么（2026-08-19 用户重新定义，之前那句概括把人带偏过多次）**：
  ...
  **（2026-08-28 用户再拍板，两条新约束）：**

  1. **服务器是「我的服务器」，不是「Windows」。** 用户明确说了将来可能换
     Mac mini 当服务器。所以**代码和界面里都不该出现「Windows」这个概念**，
     只能有一个可配置的服务器地址。
     ⚠ 这条不是洁癖 —— 今天用户抱怨的「太混乱了，这么多没用的按钮」，
     根源正是把 **Pi** 这个实现细节当成了一等公民（「登录 Pi」「Pi 书库」
     「与 Pi 同步」）。换一台机器时要改的应该是一行配置，不是把一个词
     从几十个文件里挖出来。

  2. **本地的书必须先上传服务器才能开始使用。**
     它买到的是一个**不变量**：任何一本能用的书，两边都有。
     用户自己点出了它的精髓 —— 从服务器下载的书天然满足，本地导入的书
     上传后也满足，**两条路收敛成同一个状态**，那个「两批书各是各的」问题
     从根上消失，而不是靠事后同步去补。
     ⚠ 代价要说清：服务器没开时新书打不开。现阶段服务器是 Windows（会关机），
     所以这个代价是真实的；换成常开的 Mac mini 之后才消失。

  ⚠ **今天（2026-08-28）我自己违反过上面那条 2026-08-25 的决定**：
  给 Pi 新加了语音中继、手表 token 端点、**卡片桥**。前两条是传输中继
  （手表够不到 tailnet，而 Windows 暴露到公网会变成无鉴权入口），勉强站得住；
  **卡片桥让 Pi 开始经手卡片内容，属于「新的数据链路」，跟这条决定直接冲突，
  应当迁到服务器侧。** 记在这里而不是删掉，是因为它解释了用户看到的那份混乱
  从哪来。
...
- ⚠ **TestFlight 上传上限 = 滚动 24 小时内 20 次**（altool 90382）。
  **不是**"每天 N 次"，也**不会**在午夜重置 —— 它是个滑动窗口。
  2026-08-28 实测三点全部吻合：最后一次成功时窗口内 **20**，
  首次被拒 **21**，两小时后再拒仍是 **21**。（Apple 论坛上别人报的也是
  "二十来次之后开始拒"。）

  这解释了所有看着反常的现象：08-27 一天传 18 次没事、08-28 才 13 次就撞墙
  （窗口里还装着前一天傍晚的）；被拒后"重试有时就过"不是节流放行，
  而是**恰好有一次老上传满 24h 掉出了窗口**。

  **撞上之后不要连发**（白烧构建，还看不出是配额还是别的）。正确做法是
  算出下一个名额什么时候空出 —— 只需等**最老的那次**出窗，不用等一天：

  ```bash
  gh run list --workflow=safari-extension-ios.yml --limit 80     --json conclusion,createdAt     --jq '.[] | select(.conclusion=="success") | .createdAt'
  # 取最近 24h 内的成功次数 n；n>=21 则第 (n-19) 老的那次 +24h 就是下一个名额
  ```

  它**只在上传那一步报，编译是全绿的** —— `gh run watch` 说 failure 时
  先看是不是这条，别去翻代码。

  ⚠ **我为这条结论连错两次**（先"每天 6 次"、后"短时节流重试就过"），
  根因都一样：**拿"重试成功过"当成"没有上限"的证据**。
  在边界附近成功过什么也不能证明 —— 真正的判据是把每次尝试放回时间轴上数。

  **另外，真正该改的不是重试策略，是别把 TestFlight 当调试循环用** ——
  一次发版要把所有诊断出口一起打开。08-28 同一个 bug 发了四版，
  前两版都因为诊断只加了一半而白费，配额就是这么烧光的。
