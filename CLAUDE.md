...
## 授权（2026-08-18 用户明确）
- ⚠ **TestFlight 上传每天有配额，实测落在 13–15 次之间**（altool 90382）。
  两天的数据：08-27 成功 15 次、08-28 成功 13 次之后**再也传不上去** ——
  最后一次成功 11:36(UTC)，此后 12:04→14:05 连续 8 次全拒，跨度两小时以上。

  ⚠ **我为这条结论错了两次，记下来免得再错第三次**：
  先写成"每天 6 次"（只看了一次失败就下结论），
  又改成"短时节流、重试就过"（那两次重试确实过了 —— 但那是**还没到上限**，
  不是节流放行）。**在边界附近成功过，不等于没有上限。**
  真正的判据是：**隔了几小时再试仍然被拒**，那就是配额，重发只是浪费。

  它**只在上传那一步报，编译是全绿的** —— `gh run watch` 说 failure 时
  先看是不是这条，别去翻代码。
  ⚠ **不要挂自动重连发**：撞上之后连发只会白烧构建，还看不出是配额还是节流。

  **所以真正该改的不是重试策略，是别把 TestFlight 当调试循环用** ——
  一次发版要把所有诊断出口一起打开，而不是"先看看这一处"。
  08-28 同一个 bug 发了四版，前两版都因为诊断只加了一半而白费。
