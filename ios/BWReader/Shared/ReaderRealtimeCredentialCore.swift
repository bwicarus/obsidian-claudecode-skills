...
enum ReaderRealtimeOpenAIClient {
    ...
    private static func localSessionConfiguration() -> [String: Any] {
        ...
        let tools: [[String: Any]] = [
            ...
            localTool(
                "see_ink",
                "查看当前显示页新笔迹附近的实际页面合成图；结果会同时附带冻结的同页文字、选区及前后页上下文。当前页存在本次通话中尚未看过的新笔迹变化，或用户提到当前页圈画、手写、箭头、算式时，本轮直接且只调用此工具，不要再组合 read_page 或 see_page；最终回答仍须综合图像与所附文字上下文，不要自动追逐其它页面的未读笔迹。",
                visual
            ),
            localTool(
                "see_page",
        ...
        一旦调用视觉工具，同一用户轮只选择一个视觉目标；等待工具结果后只生成一次最终回答，不要在工具前先回答，也不要再调用第二个视觉工具。工具选择唯一不代表证据只有图：最终回答必须综合工具结果所附的当前问题、此前对话、前页/当前页/后页文字、选区与合成图；图像判断圈画、外观和空间关系，文字判断人物身份、台词和语义，绝不能只描述图片。
