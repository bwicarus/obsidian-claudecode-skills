...
enum ReaderRealtimeOpenAIClient {
    ...
    private static func localSessionConfiguration() -> [String: Any] {
        ...
        let tools: [[String: Any]] = [
            ...
            localTool(
                "see_ink",
                "查看当前显示页新笔迹附近的实际页面合成图。此工具只送入图像，不重复附带当前页全文、前后页文字或选区。当前页存在本次通话中尚未看过的新笔迹变化，或用户提到当前页圈画、手写、箭头、算式时，本轮直接且只调用此工具，不要再组合 read_page 或 see_page，也不要自动追逐其它页面的未读笔迹。",
                visual
            ),
            localTool(
                "see_page",
        ...
        一旦调用视觉工具，同一用户轮只选择一个视觉目标；等待工具结果后只生成一次最终回答，不要在工具前先回答，也不要再调用第二个视觉工具。see_ink 的工具结果只有合成图，不会重复附带整页或前后页文字；直接依据图像与本轮既有对话回答。see_page 与 see_figure 可附带必要的页面上下文，仍需综合图像与该上下文。
