...
def _card_update_note(local_id: str, pairs: list) -> dict:
    ...
    prompt = (
        ...
        "有个地方没看懂，于是问了问题(③)；AI 回答里我**勾选为有用**的部分(④)恰好把"
        "这个困惑讲清楚了。也就是说，③暴露出我原来在笔记某处存在理解缺口，而④补上了它。\n\n"
        "任务：判断『③暴露的是笔记①里哪一处的理解缺口』，把『④中真正解决这个困惑的关键点』"
        "补到笔记的那个对应位置——目标是让以后读到这里的人（包括我自己）不再卡在同一个地方。\n"
        "**严格约束**：\n"
        "1. 新增内容**只能来自④**，且只取④里针对该困惑的关键点；**不得**添加④中没有的"
        "类比、例子、引申、额外阐述或你自己的发挥。\n"
        "2. ②③仅用于定位与理解我的困惑（判断补到哪一节、怎么衔接），**不作为新增内容来源**。\n"
        "3. 补在与③的困惑最相关的位置；措辞可微调以连贯，但信息严格来自④、不增不改其义；"
        "已有内容已说清的就不要重复。\n"
        "4. 改动小而精准：能在已有段落补一两句就别新增小节。\n"
        "5. 保留 frontmatter（开头 --- 之间）和「相关笔记」节原样不动；"
        "保持 Obsidian Markdown，数学公式用 $...$ 或 $$...$$。\n"
        "6. 直接输出修改后的完整笔记内容，**第一行就是笔记原本的开头（--- 或正文）**，"
        "不要写任何说明、前言或代码围栏。\n\n"
        f"=== ① 当前笔记 ===\n{original}\n\n"
        f"=== ② 正在复习的 Anki 卡片（仅用于定位，不作内容来源）===\n{card_desc}\n\n"
        f"=== ③我问的问题 + ④有用的回答（**唯一的新增内容来源**）===\n{_pairs_text(pairs)}"
    )
    try:
