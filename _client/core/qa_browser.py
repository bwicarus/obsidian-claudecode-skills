...
def _card_update_note(local_id: str, pairs: list, verbosity: str = "verbose") -> dict:
    ...
    if rec_file and rec is not None:
        ...
        try:
            ...
            pass
    # 显示新增字数 + 相对于 ④ 选中内容的保留率（用户能直观看到 AI 是否过度压缩）
    ...
    warn = ""
    if pairs_text_len > 200 and retention < 0.4:
        warn = "（⚠️ 保留率偏低，AI 可能压缩过多，建议检查或重跑）"
    summary = (f"笔记已更新（{note_path.name}），新增约 {delta_chars} 字 / "
