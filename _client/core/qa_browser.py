...
def _card_update_note(local_id: str, pairs: list) -> dict:
    ...
    prompt = (
        ...
        "我正在复习的 Anki 卡片；③我复习时问的问题；④我**勾选为有用**的 AI 回答。\n\n"
        "任务：把【④有用回答】里的知识点融入笔记最相关的位置。**严格约束**：\n"
        "1. 只能补充【④有用回答】中实际出现的内容；**不得**自行添加有用回答里没有的"
        "类比、例子、引申、额外阐述或你自己的发挥。\n"
        "2. 卡片①②③仅用于帮你判断【该补到笔记的哪一节、如何与上下文衔接】，"
        "**不作为新增内容的来源**。\n"
        "3. 措辞可调整以与原文风格连贯、避免逐字粘贴，但信息必须严格来自④，"
        "不增不改其含义；与笔记已有内容重复的就不要再加。\n"
        "4. 改动尽量小而精准：能在已有段落补一两句就别新增小节。\n"
        "5. 保留 frontmatter（开头 --- 之间）和「相关笔记」节原样不动；"
        "保持 Obsidian Markdown，数学公式用 $...$ 或 $$...$$。\n"
        "6. 输出修改后的完整笔记内容，不要任何说明或代码围栏。\n\n"
        f"=== ① 当前笔记 ===\n{original}\n\n"
        f"=== ② 正在复习的 Anki 卡片（仅用于定位，不作内容来源）===\n{card_desc}\n\n"
        f"=== ③问的问题 + ④有用的回答（**唯一的新增内容来源**）===\n{_pairs_text(pairs)}"
    )
    try:
