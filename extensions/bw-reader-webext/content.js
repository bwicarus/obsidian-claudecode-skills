...
// Page text for the voice call. The call runs in an extension page of its own
// (it has to: only that origin may dial the bridge directly), and that page
// cannot see the tab the user was reading. So the popup asks here, and this
// replies with what the assistant needs to talk about the page.
//
// Read-only and answer-only: it never initiates, and it exposes nothing the
// extension could not already read through its content script.
(function () {
  "use strict";
  var runtime = (typeof chrome !== "undefined" && chrome.runtime) || null;
  if (!runtime || typeof runtime.onMessage === "undefined") return;

  // Enough for the assistant to discuss the page, bounded so a huge document
  // cannot blow past the bridge's 64 KiB message ceiling.
  var MAX_TEXT = 12000;
  ...
  function visibleText() {
    ...
    var text = "";
    try {
      // innerText, not textContent: it respects display:none and collapses the
      // way the page actually reads, so hidden nav/script noise stays out.
  ...
  runtime.onMessage.addListener(function (message, _sender, sendResponse) {
    if (!message || message.type !== "BW_VOICE_PAGE_CONTEXT") return undefined;
    var selection = "";
    try {
