...
> **2026-09-22 用户拍板：Chromium 作为产品表面正式退掉。** 只做 Safari
> （iOS/macOS，随 App 一起发）。
>
> 这意味着：不再声称支持 Chrome、不再为它做兼容取舍、门禁**不为它绿**。
> 以后看到「在 Chrome 上不工作」这类问题，正确反应是「那不是我们的表面」，
> 而不是去修。
>
> ⚠ **但 Playwright 的 Chromium 测试跑道保留**，别顺手一起删了 ——
> 它在这里是个**浏览器引擎替身**，跑的是 Safari 扩展**也在用**的那份 JS
> （扩展自己那套 IndexedDB 仓库、选区命中等）。拆掉的话这些代码就一条
> 自动化测试都没有了。Safari 的 IndexedDB 与 Chromium 的不是同一个实现，
> 所以它本来就只是近似 —— 但近似的测试远好过没有，尤其在存储上
> （2026-09-22 那个 NUL 主键 bug 就是"真跑才露馅"）。
